...
{
  # setting the correlation coefficient between x1 and x2
...
demonstrate_representations_3D <- function(model, n, cue1_range, cue2_range, cue_names, lambda, pi) {
  ...
  for (i in 1:length(model$category)) {
    ...
    # f.bivn(x, y, mu, Sigma)
    # mvtnorm::dmvnorm(cbind(x,y), mu, Sigma)
    # Rfast::dmvnorm(cbind(x,y), mu, Sigma)
    # 
    # z <- t(outer(x, y, FUN = f.bivn, mu, Sigma)) 
    z <- t(outer(x, y, FUN = bivariate_density, mu, Sigma)) 
    # NOTE: SOMETHING IS NOT QUITE RIGHT WITH THE FUNCTION ABOVE. THE Y-AXIS GETS LABELED WEIRDLY AND THE VALUES SHOW UP AS 600\MU, ETC.
