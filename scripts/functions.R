...
demonstrate_representations_3D <- function(model, n, cue1_range, cue2_range, cue_names, lambda, pi) {
  for (i in 1:length(model$category)) {
    ...
    if (category == model$category[1]){
      d.bivn <- df
      rho <- Sigma[3] / (sqrt(Sigma[1]) * sqrt(Sigma[4]))
      # add ellipse at the bottom
      d.ellipse <- mixtools::ellipse(
        mu, 
        matrix(
          c(
            Sigma[1], 
            rho * sqrt(Sigma[1]) * sqrt(Sigma[4]), 
            rho * sqrt(Sigma[1]) * sqrt(Sigma[4]), 
            Sigma[4]), 
          2), 
        alpha = .05, 
    ...
    if (category == model$category[2]){
      t.bivn <- df
      # add ellipse at the bottom
      t.ellipse <- mixtools::ellipse(
        mu, 
        matrix(
          c(
            Sigma[1], 
...
plot_3D.categorization.diff <- function(df.resp){
  # plot categorization surface
  p.3d.categorization <- plot_ly(width = p_width, height = p_height) %>%
    add_surface(x = df.resp$x, y = df.resp$y, z = df.resp$d_diff,
                opacity = 0.5,
                # showlegend = FALSE,
                showscale = FALSE,
