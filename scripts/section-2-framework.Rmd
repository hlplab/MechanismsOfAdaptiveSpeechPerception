...
Many accounts of normalization assume further transformation of the acoustic input based either on other concurrently occurring acoustic properties (intrinsic normalization) or on the overall distribution of the acoustic input itself [extrinsice normalization, for review, see @weatherholtz-jaeger2016]. For example, some of the most influential normalization accounts for vowel formants either center [@nearey1989] or both center and standardize formants based on the talker-specific mean and standard deviation [@lobanov1971; @gerstman1968]. These types of accounts have been found to remove a substantial amount of cross-talker variability in the realization of vowels [e.g., @hoffmanbion-escudero2007; @nearey1989; @persson-jaeger2023; for critical review, see @barreda2020]. In particular, accounts that assume simple centering seem to provide a good fit against vowel categorization by human listeners [e.g., @barreda2021; @persson-jaeger2022; @richter2017].
...
if (ANIMATE_FIGURES) {
  ...
  if (SAVE_ANIMATED_VIDEO) {
    anim_save("demonstrate-changes-in-representations.webm", animation = animate(p.animation, renderer = av_renderer(), width = 1280, height = 720), path = '../figures/knitted/')
  }
} else {
...
if (ANIMATE_FIGURES) {
  ...
  p.animation
  if (SAVE_ANIMATED_VIDEO) {
    anim_save("demonstrate-changes-in-representations-b.webm", animation = animate(p.animation, renderer = av_renderer(), width = 1280, height = 720), path = '../figures/knitted/')
  }
} else {
  d.plot <- d.exposure %>% filter(observation.n == max(observation.n))
  p %+% d.plot
...
if (ANIMATE_FIGURES) {
  ...
  p.animation
  if (SAVE_ANIMATED_VIDEO) {
    anim_save("demonstrate-lapse-bias-change.webm", animation = animate(p.animation, renderer = av_renderer(), width = 1280, height = 720), path = '../figures/knitted/')
  }
} else {
  d.plot <- d.exposure %>%
...
if (ANIMATE_FIGURES) {
  ...
  p.animation
  if (SAVE_ANIMATED_VIDEO) {
    anim_save("demonstrate-lapse-bias-change-nonzero-lapse.webm", animation = animate(p.animation, renderer = av_renderer(), width = 1280, height = 720), path = '../figures/knitted/')
  }
} else {
  d.plot <- d.exposure %>%
  mutate(use_logit = factor(ifelse(use_logit, "log-odds", "probability"), levels = c("probability", "log-odds"))) %>% filter(observation.n == max(observation.n))
