...
\caption{Illustrating how ASP can be used to derive predictions (or to fit to human behavior). Many of ASP's parameters can be fixed based on phonetic databases that are used as stand-in for listeners' longterm experience prior to the experiment. This reduces the degrees of freedom and functional flexibility of ASP's change models, constraining the predictions. After updating the ASP model based on the inputs experienced during exposure, the updated categorization model can be used to predict listeners' response distributions for any particular test token (as we do in Case Study 1), and/or to predict listeners' recognition accuracy (as we do in Case Study 2).}\label{fig:ASP-predictions}
\end{center}
\end{figure}
