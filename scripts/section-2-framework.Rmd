...
d.exposure <- 
  ...
  crossing(use_logit = c(T, F)) %>%
  unnest(posterior) %>%
  nest(posterior = -c(beta, use_logit, starts_with("observation"))) %>%
  select(-data)
