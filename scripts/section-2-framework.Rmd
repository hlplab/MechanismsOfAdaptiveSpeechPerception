...
p <-
  ...
  ggh4x::force_panelsizes(cols = duo.panel.width, rows = duo.panel.height) +
...
```{r demonstrate-lapse-bias-change, fig.cap="(ref:demonstrate-lapse-bias-change)", fig.height=base.height + .5, fig.show=if (ANIMATE_FIGURES) 'animate' else 'asis', fig.width=base.width*2.5 + 1.5, message=FALSE, warning=FALSE}
d.exposure <-
  d.exposure.demo %>%
  crossing(
    beta = c(0.1, 1, 10),
