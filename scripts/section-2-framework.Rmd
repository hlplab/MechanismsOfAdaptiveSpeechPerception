...
p.duo <- plot_grid(
  ...
  axis = "lrtb")
p.duo
ggsave(paste0('../figures/plotly/demonstrate-production-perception-link.png'), plot = p.duo, width = base.width * 2 + 1, height = base.height * 1 + 1, dpi = 300)
```
...
ggsave(paste0('../figures/plotly/demonstrate-normalization.png'), plot = p1, width = phonetic.panel.width*3 + 1, height = phonetic.panel.height*2 + 3, dpi = 300)
```
...
```{r demonstrate-changes-in-representations, fig.width=base.width*2 + .5, fig.height=base.height +.5, fig.cap="(ref:demonstrate-changes-in-representations)", fig.show='animate'}
p <- 
  d.exposure %>%
...
if (ANIMATE_FIGURES) {
  ...
  ggsave(paste0('../figures/plotly/demonstrate-changes-in-representations.png'), plot = p1, width=base.width*2 + .5, height=base.height +.5, dpi = 300)
...
```{r demonstrate-changes-in-representations-b, fig.width=base.width*2 + .5, fig.height=base.height +.5, fig.cap="(ref:demonstrate-changes-in-representations-b)", warning=FALSE, message=FALSE, fig.show=if(ANIMATE_FIGURES) 'animate' else 'asis'}
p <- d.exposure %>%
  ggplot(aes(x = !! sym(cue.demo))) +
  # categorization function
  geom_line(
...
if (ANIMATE_FIGURES) {
  ...
  ggsave(paste0('../figures/plotly/demonstrate-changes-in-representations-b.png'), plot = p1, width=base.width*2 + .5, height=base.height +.5, dpi = 300)
