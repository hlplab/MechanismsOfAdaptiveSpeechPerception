...
Beyond these general similarities, however, the three change models are subject to different computational limitations that affect their functional flexibility. An obvious example of such a limitation is the fact that changes in decision-making can only predict changes in the  This means that, for some types of exposure, there *are* changes in the mapping from stimulus to categorization that some of the change models can predict while others cannot. If adequate approaches to data analysis are chosen---approaches that shed light on the changes in the mapping from acoustics to listeners' responses---it is possible to distinguish between the three different change mechanisms (and even combinations of them). Before we discuss what would be required to achieve this goal, we present the second case study using another very common experimental paradigm.
