...
p.PR.exposure <- d.PR.exposure %>%
  ...
  scale_y_continuous(expression("f0 (Mel)"), expand = expansion(mult = .1, add = 0),
                     breaks=seq(180, 280, by = 20)) +
  coord_cartesian(ylim = c(180, 280)) +
...
```{r study-models-common-functions}
...
```{r study-models-changes-in-decision-making-functions, echo=FALSE, message=FALSE, warning=FALSE}
...
prow.B <- plot_grid(
    p.PR.representations.highlighted + theme(legend.position="none"),
    p.PR.bias.highlighted + theme(legend.position="none"),
    p.PR.normalization.highlighted + theme(legend.position="none"),
    ncol = 3, 
    rel_widths = c(0.25, 0.25, 0.23))
prow <- 
  plot_grid(
    p20 + myGplot.defaults(base_size = 14, set_theme = F) + theme(legend.position="none", plot.margin = margin(t = 1.3, r = 1, b = 0.7, l = 0, unit = "cm")), 
    prow.B,
    labels = c('A)', 'B)'), 
    ncol = 2, 
