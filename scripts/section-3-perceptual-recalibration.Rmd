...
All of these findings *could*, however, be compatible with explanations in terms of any of the three change mechanisms introduced in Section \@ref(sec:framework). The goal of Case Study 1 is thus to assess the degree to which the typical findings from perceptual recalibration experiments do or do not distinguish between the three mechanisms. Additionally, we hope that our case studies will serve as a template for other researchers interested in applying ASP models to the interpretation of their experiments [see, e.g., @theodore-monto-2019 for an application of the representational change model].

Unfortunately, most published experiments on perceptual recalibration do *not* report the acoustic properties of their stimuli. Some studies provide aggregate information [e.g., @kraljic-samuel2006; @kraljic-samuel2007] and very few provide detailed information and visualization [e.g., @drouin2016]. 
...
All three change models start from the same prior state. Specifically, we assume that listeners have acquired category representations with expected category means ($\mathbf{E}(\mu_{/d/}), \mathbf{E}(\mu_{/t/})$) and variances ($\mathbf{E}(\sigma_{/d/}), \mathbf{E}(\sigma_{/t/})$) that reflect the talker-normalized distribution in Chodroff and Wilson (2018; see Figure \@ref(fig:show-representations-plots)).^[Category priors are generated using C-CuRE normalized category means and covariance matrices obtained from the data in @chodroff-wilson2018. Of note, unlike what is typically adopted in experiments, the data from Chodroff & Wilson (presented in Figure \@ref(fig:demonstrate-normalization)) was highly unbalanced, with two times more /d/ tokens than /t/. This creates an interesting issue for normalization accounts which we will discuss in the result section. For the simulations in Case Studies 1 and 2, we simply sub-sampled from the original corpus data to create a balanced dataset where equal numbers of /d/ and /t/ tokens are included. The details of this implementation are also described in the SI (\@ref(sec:SI-PR)).] Prior to any exposure, decision-making is assumed to employ uniform response biases ($\pi_{/d/}=\pi_{/t/}=.5$) and exhibit zero lapse rates. The findings we present below do not qualitatively depend on these specific assumptions. For example, none of the qualitative patterns would change if listeners had non-uniform response biases or non-zero lapse rates at the start of the experiment.
