...
Finally, Figure \@ref(fig:PR-result-changes-in-representations) shows that distributional learning not only predicts shifts in the category boundary but can also predict changes in the slope of those boundaries. This is evident, for example, in the top-right panel. Although such changes in slopes are often not discussed, they are present in many perceptual recalibration experiments [see @drouin2016; @liu-jaeger2018; @liu-jaeger2019; @myers-mesite2014].

### Changes in decision-making
Next, we model the effects of changes in decision-making. Such changes have rarely been considered as an explanation for boundary shifts [but see @clarkedavidson2008]. In addition to varying the rate at which response biases change $\beta_{\pi}$, we also vary the lapse rate parameter $\lambda$. Normalization and representations did not change based on exposure ($\kappa_0 = \kappa_{0,c} = \nu_{0,c} = \infty$). 
...
```{r PR-test-normalization, fig.width= base.width * 2, fig.height= base.height * (if (animate) 1 else 3) + (if (animate) 1 else 0), fig.cap="(ref:PR-test-normalization)", fig.show= if (animate) 'animate' else 'asis'}
d.PR <- d.PR.exposure %>%
...
To anticipate the general discussion, this does not mean that perceptual recalibration experiments are *necessarily* uninformative about the mechanisms underlying the effects of recent exposure. While the three change models are sufficiently flexible to explain a wide range of changes in categorization responses, the models' predictions *are* constrained by the acoustic properties of the exposure stimuli---specifically, by the relation between those stimuli and listeners' prior expectations based on previously experienced speech input. Perhaps the most obvious evidence of such constraints is that none of the change models predicts 'anti-adaptation' under any parameter settings. Rather, all three models adjust categorization responses in a way that moves the categorization function in the 'right' direction, compared to the absence of an adaptive response (this will become more evident in Case Study 2, where we show that the adaptive responses predicted by all three change models improve recognition accuracy).

Beyond these general similarities, however, the three change models are subject to different computational limitations that affect their functional flexibility. Recall, for example, from Section \@ref(sec:framework) that normalization affects cue values regardless of category membership, or that changes in decision-making can only predict additive changes in the log-odds of categorization responses when listeners have lapse rates of 0 or 1. As we demonstrate in the next section, such limitations mean that, for some types of exposure, there *are* changes in the mapping from stimulus to categorization that some of the change models can predict while others cannot. We return to this point in the general discussion. If adequate approaches to data analysis are chosen---approaches that shed light on the changes in the mapping from acoustics to listeners' responses, it is thus possible to distinguish between the three different change mechanisms (and even combinations of them). Before we discuss what would be required to achieve this goal, we present the second case study using another very common experimental paradigm.
