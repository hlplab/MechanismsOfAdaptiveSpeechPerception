...
## Summary
We applied the three change models developed in Section \@ref(sec:framework) to a (simulated) experiment within the perceptual recalibration paradigm. We find that all three change models can qualitatively predict the type of boundary shift characteristic to such experiments. At least at the level of analysis that is typically applied in these experiments, the signature result of perceptual recalibration experiments therefore do *not* constitute decisive evidence for changes in representations [contrary to what is often assumed in previous studies since @norris2003]. Rather, boundary shifts could result from *any* of the three types of change mechanisms. 

To anticipate the general discussion, this does not mean that perceptual recalibration experiments are *necessarily* uninformative about the mechanisms underlying the effects of recent exposure. If adequate approaches to data analysis are chosen, perceptual recalibration experiments do, in principle, provide information that can be used to distinguish between the three explanations. Before we discuss ways towards studies that more decisively distinguish between the three explanations, we present a second case study using another very common paradigm.
