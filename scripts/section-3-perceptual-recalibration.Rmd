...
p.PR.results %+% 
  ...
  geom_text(
    data = . %>%
      group_by(Condition, posterior.lapse_rate, beta_delta_epsilon) %>%
      summarise(
    ...
    show.legend = F) +
  facet_grid(
    posterior.lapse_rate ~ beta_delta_epsilon, 
    labeller = label_bquote(
      cols = beta[pi] == .(beta_delta_epsilon),
...
#          posterior.lapse_bias1 = map(posterior, ~ .x %>% filter(category == categories.PR[1]) %>% pull(lapse_bias)),
#          posterior.lapse_bias2 = map(posterior, ~ .x %>% filter(category == categories.PR[2]) %>% pull(lapse_bias))) %>%
#        group_by(Condition, beta_delta_epsilon) %>%
#        mutate(
#          lapse_bias_expression = list(
#            bquote(
#              atop(
#                beta[delta[pi]] == .(beta_delta_epsilon),
#                atop(
