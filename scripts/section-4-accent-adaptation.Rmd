...
d.AA.normalization <- d.AA.exposure %>% 
  nest(data = -c(Condition, Subject)) %>%
  crossing(
...
temp1 <- readRDS("../models/best_performing_parameters.bias.rds")
temp2 <- readRDS("../models/best_performing_parameters.bias_Cue_reweighting.rds")
...
```{r show-AA-models-best-three-cue-reweighting, eval=FALSE}
dist.L2_category.cue1 <- 1
dist.L2_category.cue2 <- 0.22
shift.cue1 <- 0.5
shift.cue2 <- 0.22
...
example_label = "Cue_reweighting"
d.AA.results.cue_reweighting  <- construct_three_models(example_label = "Cue_reweighting", experimenter.ideal_observer = m.io.VOT_f0.AA)

d.AA.results.cue_reweighting.best <- select_three_best_models(d.AA.results.contrast_shift, example_label = "Cue_reweighting")
```
...
```{r AA-result-comparison-best-three-models-contrast-collapse, warning=FALSE, eval = FALSE}
...
```{r AA-result-comparison-additional-cases, warning=FALSE, eval = FALSE}
...
```{r AA-plot-target-exposure-additional-cases, message = FALSE, warning=FALSE, eval = FALSE}
...
```{r AA-plot-results-additional-cases, fig.width=base.width*4 + .5, fig.height=base.height*3 + .5, fig.cap = "(ref:AA-plot-results-additional-cases)", warning=FALSE, eval = FALSE}
...
```{r AA-table-best-performing-parameterization, eval = FALSE}
parameters.results.best.cases <- bind_rows(best_performing_parameters.representations.par,                      best_performing_parameters.bias.par, best_performing_parameters.normalization.par) %>%
  mutate(parameterization = case_when(
   ModelType == "representations" ~ paste0("kappa = ", prior_kappa, ", nu = ", round(prior_nu,digits = 2)),
   ModelType == "decision making" ~ paste0("lambda = ", lapse_rate, ", beta = ", round(beta_pi, digits = 2)),
   ModelType == "normalization" ~ paste0("kappa = ", prior_kappa))) %>%
   mutate(
      ModelType = factor(paste0("Changes in ", ModelType), levels = c("Changes in representations", "Changes in decision making", "Changes in normalization"))) %>%
  #  mutate(Label = interaction(ModelType, parameterization, sep = "\n")) %>%
