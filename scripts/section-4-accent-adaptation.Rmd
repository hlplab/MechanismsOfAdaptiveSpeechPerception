...
# Case 1: presented in the main text, with clear cue-weighting, representing cases like word-final stops. 
dist.L2_category.cue1 <- 0.5
dist.L2_category.cue2 <- 0.7

# Case 2: no cue-weighting, but greater category overlap due to reduced distance, representing cases like s-th
# dist.L2_category.cue1 <- 0.5
...
```{r study-AA-exposure-plot, fig.width=base.width*2 + 1, fig.height=base.height, fig.cap="(ref:study-AA-exposure-plot)", eval = FALSE}
...
d.AA.test <- make_accent_adaptation_test_design(d.AA.exposure, experimenter.ideal_observer = m.io.VOT_f0.AA) 
```
...
```{r study-AA-test-plot, fig.width=base.width*2 + 1, fig.height=base.height, fig.cap="(ref:study-AA-test-plot)", eval = FALSE}
...
It should be emphasized that the relative benefit of L2-accented exposure predicted by each of the change mechanisms depends on the stimulus statistics of the (simulated) experiment. Below we present a different case simulating a scenario where the category realizations in the L2 accent have a similar cue weighting to that in the L1 accent but exhibit greater category overlap. Using the exact same range of parameters considered above, the predicted performance of the L2-accented exposure shows a different pattern. Specifically, here changes in decision making are predicted to yield greatest benefits, followed by the normalization change model and then the representational change model. Counter-intuitive at first sight, the results again highlight that any of the three change mechanisms may explain even complex adaptive behavior, including adaptation to naturalist accents. Critically, while representational changes are often thought to underlie effective accent adaptation, it is predicted that they actually lead to the least overall performance, compared to computationally simpler changes in decision making or normalization. Whether listeners indeed employ different strategies as a function of the stimulus features is empirically testable. Next, we discuss the consequences of these findings and provide recommendations for future research.
...
(ref:AA-result-comparison-best-three-models) A simulated accent adaptation experiment representing another type of L1-L2 accent difference. Predictions of a model that derives accent adaptation as changes in decision-making. Predicted categorization responses are shown for the test tokens after `r conditions.AA[1]` and `r conditions.AA[2]` exposure, depending on the rate at which response biases change ($\beta_{\pi}$) and the rate of attentional lapses ($\lambda_{posterior}$). The highlighted panel corresponds to the combination of $\beta_{\pi}$ and $\lambda_{posterior}$ resulting in the best overall accuracy. 

**Panel A - Exposure:** Distribution of the stimuli used during the exposure phase of the accent adaptation experiment (`r n.exposure.token` tokens each of `r conditions.AA[1]` and `r conditions.AA[2]` `r paste(categories.AA, collapse = " and ")`, respectively). **Panel B - Test:**  Distribution of the stimuli used during the test phase of the accent adaptation experiment. The test tokens come from L2-accented speech (`r n.test.token` tokens per category) and are identical for the two exposure conditions. Ellipses show the 95% probability mass for the two categories in L2-accented exposure speech. **Panel C - Change model predictions:** Predicted categorization accuracies are shown for the L2-accented test tokens after L1-accented and L2-accented exposure. The average accuracy across all test tokens for each condition is shown above the bars. Error bars show 95% bootstrapped confidence intervals. The parameter settings are identical to those shown in Case Study 2. The parameter combinations that result in the best overall accuracy in each model are shown.

```{r AA-result-comparison-best-three-models,fig.width=base.width*3 + .5,  fig.height=base.height*2 + .5, fig.cap="(ref:AA-result-comparison-best-three-models", warning=FALSE}
...
d.AA.results.best  <- construct_three_best_models(experimenter.ideal_observer = m.io.VOT_f0.AA)
p.top <- Make_exposure_test_plot(exposure.data = d.AA.exposure, test.data = d.AA.test)

p.bottom <- d.AA.results.best %>%
ggplot(aes(x = Condition, y = response, fill = Item.Intended_category, alpha = Condition)) +
    stat_summary(fun = mean, 
                 geom="bar", position = position_dodge(0.6),
                 width = 0.6) +
...
plot_grid(p.top, p.bottom, labels = c('', 'C)'), ncol = 1, rel_heights = c(1, 1), rel_widths = c(2,1))

# d.AA.results.best <- bind_rows(
#   d.AA.representations %>% filter(category == Item.Intended_category & prior_kappa == 4 & prior_nu == 1024) %>% mutate(ModelType = "representations"), 
#   d.AA.bias %>% filter(category == Item.Intended_category & posterior.lapse_rate == min(d.AA.bias$posterior.lapse_rate) & beta_pi == max(d.AA.bias$beta_pi)) %>% mutate(ModelType = "decision making"),
#   d.AA.normalization %>% filter(category == Item.Intended_category & normalization == "centered based\non exposure" & prior_kappa.normalization == min(as.numeric(as.character(d.AA.normalization$prior_kappa.normalization)))) %>% mutate(ModelType = "normalization")) %>%
#   mutate(ModelType = factor(ModelType, levels = c("representations", "decision making", "normalization")))
