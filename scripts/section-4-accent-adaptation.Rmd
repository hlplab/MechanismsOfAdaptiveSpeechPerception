...
<!-- TO DO: have a table here that summarizes the five parameters for the three change models, along with the min and max values considered for them. -->
...
best_performing_parameters.representations <- 
  optim(
    ...
    lower = c(log(min(range.prior_kappa)), log(min(range.prior_nu))),
    upper = c(log(max(range.prior_kappa)), log(max(range.prior_nu))), 
    control = list(
      trace = 1, 
...
# Transform parameters back into counts (from log counts)
...
get_accuracy_from_updated_bias <- function(
  ...
  # TO DO: this might have to be run 20-100 times while scrambling exposure to get a mean accuracy?
  suppressWarnings(
    update_NIW_response_bias_incrementally(
      prior = model %>% 
...
best_performing_parameters.bias <- 
  optim(
    ...
    par = c(mean(range.lapse_rate), log(mean(range.beta_pi))), 
    fn = get_accuracy_from_updated_bias,
    # The only method that works for multiple parameters with bounds
    method = "L-BFGS-B",
    # Stop searching outside of those bounds
    lower = c(min(range.lapse_rate), log(min(range.beta_pi))),
    upper = c(max(range.lapse_rate), log(max(range.beta_pi))), 
    control = list(
      trace = 1, 
      fnscale = -1,   # negative scale since we are seeking to maximize accuracy
