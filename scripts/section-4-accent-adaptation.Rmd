...
update_bias_and_categorize_test <- function(
  ...
  test,
  cues = c("VOT", "f0_Mel"),
  n.simulations = n.simulations_per_update
) {
  lapply(
    X = as_list(1:n.simulations),
    FUN = function(x) 
      update_NIW_response_bias_incrementally(
        prior = prior,
        beta = beta_pi,
      ...
      add_test_and_categorize(test)) %>%
    reduce(bind_rows) %>% 
    group_by(!!! syms(setdiff(names(.), "response"))) %>% 
    summarise(response = mean(response), response.se = sd(response) / sqrt(sum(is.na(response))))
}
...
get_accuracy_from_updated_bias <- function(
  ...
  # TO DO: we should repeat this at least 20-100 times while shuffling exposure, in order deal with 
  # the fact that this updating mechanism is random (we could do so for the plot, too). This might 
  # also mean that we should save this result and load it unless override == T since it will likely
