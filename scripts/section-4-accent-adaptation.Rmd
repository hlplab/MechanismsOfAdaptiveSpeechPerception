...
```{r AA-result-comparison-best-three-models-case3, warning=FALSE}
...
```{r AA-result-comparison-best-three-models-case4, warning=FALSE}
...
min_VOT = -100 # simulating a case like French-accented English stops, so allowing negative VOTs
...
p.AA.exposure <- d.AA.exposure.target.cases %>%
  ...
  stat_ellipse(aes(color = Group), level = .95, geom = "polygon") +
    # geom_rug(data = . %>%
    #              group_by(Case, Condition, Item.Category, Group) %>%
...
d.AA.results.best.cases <- rbind(d.AA.results.best.case2 %>%
                               mutate(Case = "case2"),
                             d.AA.results.best.case3 %>%
                               mutate(Case = "case3"),
                             d.AA.results.best.case4 %>%
                               mutate(Case = "case4")
                             ) %>%
  mutate(Case = factor(Case, levels = c("case3", "case2", "case4"), 
                            labels = c("Contrast shift", "Contrast reduction", "Contrast collapse"))) 
