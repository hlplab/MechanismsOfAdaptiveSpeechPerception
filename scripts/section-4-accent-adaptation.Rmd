...
get_parameters_phonetic_contrast.AA <- function(ideal.observer, 
  ...
  # Allow a shift of nonnative category means
    mu_nonnative_A <- c(mu_nonnative_A[1]+ (mu_nonnative_A[1]+ mu_nonnative_B[1])/2*shift_mean_1, 
                        mu_nonnative_A[2]+ (mu_nonnative_A[2]+ mu_nonnative_B[2])/2*shift_mean_2)
    mu_nonnative_B <- c(mu_nonnative_B[1]+ (mu_nonnative_A[1]+ mu_nonnative_B[1])/2*shift_mean_1, 
                        mu_nonnative_B[2]+ (mu_nonnative_A[2]+ mu_nonnative_B[2])/2*shift_mean_2)
...
As noted above, we have focused here on each change model's ability to predict the qualitative results typically observed in experiments on accent adaptation. The relative benefit of L2-accented exposure predicted by each of the change mechanisms depends on the exposure and test stimuli of the (simulated) experiment, and thus---if naturally produced speech is employed---the specific nature of the non-native changes introduced by the L2 accent. To make this point more concrete, Figure \@ref(fig:AA-result-comparison-best-three-models-case2) presents the best-performing variants of each change model for a different (simulated) scenario that is common for L2 accents: the reduction or complete collapse of an L1 contrast. Compared to the scenario considered so far, the predicted performance in Figure \@ref(fig:AA-result-comparison-best-three-models-case2)C shows a different pattern. Specifically, changes in decision-making are predicted to yield the greatest absolute benefits, followed by the normalization change model and then the representational change model. Counter-intuitive at first sight, the results again highlight that any of the three change mechanisms may explain even complex adaptive behavior, including adaptation to natural accents. 

Figure \@ref(fig:AA-result-comparison-best-three-models-case2) presents yet another example of a different but common type of L1-L2 accent difference. In this case, the two categories in the L2 accent are shifted while still maintaining a good amount of distinction within the contrast [for example, characteristic of word-initial stops in French-accented English, @Sumner2011a]. Figure \@ref(fig:AA-result-comparison-best-three-models-case2)C shows that all three change models predict considerable improvements, resulting in an over 85% accuracy in the L2-accented exposure condition. This result again suggests that changes in both decision making and normalization have the potential to elicit large benefits, contrary to commonly held assumption that only representational changes can explain great behavioral improvements. Next, we discuss the consequences of these findings and provide recommendations for future research.
...
construct_three_best_models <- function(
  ...
  d.AA.test <- make_accent_adaptation_test_design(d.AA.exposure, experimenter.ideal_observer = m.io.VOT_f0.AA, test.n_block = n.test_block, n_test = n.test.token, category_dist_ratio1 = dist.L2_category.cue1, category_dist_ratio2 = dist.L2_category.cue2, shift_mean_1 = shift.cue1, shift_mean_2 = shift.cue2)
