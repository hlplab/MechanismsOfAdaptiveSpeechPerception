...
make_exposure_test_plot(exposure.data = d.AA.exposure, test.data = d.AA.test) #COME BACK HERE TO CHECK WHY F0 is shifted too
```
...
d.AA.bias <- d.AA.exposure %>%
  nest(data = -c(Condition, Subject)) %>%
    crossing(
      lapse_rate = c(.05),
      beta_pi = c(0, .1)) %>% # specify what parameters to plot
    crossing(
      m.ia.VOT_f0.AA %>%
...
```{r compare-AA-models-best-three-functions, eval=FALSE}
...
construct_three_best_models <- function(
  ...
  d.AA.bias.rank <- d.AA.bias %>%
    ...
    group_by(Condition, lapse_rate, beta_pi) %>%
    summarise(mAcc = mean(response)) %>%
    arrange(-mAcc, -lapse_rate, -beta_pi) %>%
    filter(Condition == "L2-accented")
  ...
  ggsave(paste0('../figures/plotly/AA_cases/', example_label, '_bias.png'), plot = p.AA.bias, width = 2*(base.width*length(levels(factor(d.AA.bias$beta_pi))) + .5), height = 2*(base.height*length(levels(factor(d.AA.bias$lapse_rate))) + .5), dpi = 300)
  ...
  d <- bind_rows(
    ...
    d.AA.bias %>% 
      filter(category == Item.Intended_category & lapse_rate == best_performing_case.bias$lapse_rate & beta_pi == best_performing_case.bias$beta_pi) %>%
...
```{r AA-result-comparison-best-three-models-contrast-reduction, warning=FALSE, eval = FALSE}
...
```{r AA-result-comparison-best-three-models-contrast-shift, warning=FALSE, eval = FALSE}
