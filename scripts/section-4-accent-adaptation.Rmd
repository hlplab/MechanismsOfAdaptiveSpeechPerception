...
As in Case Study 1, we first model changes in representations, and then compare them against change models in decision-making and normalization. We consider a larger range of parameterizations than in Case Study 1, because initial investigations found that the models with the highest accuracy for L2-accented exposure (henceforth, the best-performing parameterization) in some cases fell outside of the range of parameter values considered in Case Study 1. These best-performing parameterizations are marked in all result plots we present below (if multiple parameterizations yielded indistinguishable performance, then the one with the slowest learning rate is marked as 'best-performing').
...
best_performing_case.normalization.both <- 
  d.AA.normalization %>%
  filter(category == Item.Intended_category) %>%
  filter(normalization == "centered based\non exposure") %>%
...
To make this point more concrete, Figure \@ref(fig:AA-plot-results-additional-cases) presents three additional (simulated) scenarios that reflect common properties of L2 accents. The top panel shows an example of *contrast shift*, where two categories in L2-accented speech are shifted along one or more of the cue dimensions as compared to L1-accented speech [e.g., attested for word-initial stops in French-accented English, @Sumner2011a]. The middle panel shows an example of *contrast reduction*, where an L2-accent shows a greater category overlap than an L1-accent [e.g., attested for vowels in Spanish-accented English, @wade2007]. The bottom panel shows an example of *contrast collapse*, where two contrasting categories in an L1-accent overlap substantilly or completely in an L2-accent [e.g., characteristic of a loss of distinction between [s] and [`r linguisticsdown::cond_cmpl("θ")`] in Mandarin-accented English, @zheng-samuel2020]. The three scenarios are created by varying category means along VOT for the L2-accented categories, without introducing more complex variation in category variance and covariance. The L1-accented exposure condition is the same as in the scenario above (Figure \@ref(fig:study-AA-exposure-test-plot) and held constant across these three scenarios. As above, test tokens are randomly sampled from the distribution of each of the L2-accent category. Consequently, test tokens varied across the three scenarios examined here. 
