...
For L1-accented exposure, we again see that /d/ test tokens are categorized substantially less accurately than /t/ test tokens, regardless of the specific value of ($\beta_{\pi}$). The second signature result---improved accuracy after L2-accented exposure---is obtained for sufficiently large $\beta_{\pi}$.
The largest improvements are found for the smallest lapse rates and fastest change rates (gray panel in Figure \@ref(fig:AA-result-changes-in-decision-making)).

Importantly, Figure \@ref(fig:AA-result-changes-in-decision-making) also shows that changes in response biases after L2-accented exposure not only can predict improvements for /d/ but can result in *overall* improvements in accuracy, compared to L1-accented exposure (e.g., top right panel). Thus, changes in response biases do not necessarily result in the type of zero-sum trade-off---accuracy improving for one category at the cost of accuracy for the other category---that is sometimes attributed to them. Indeed, zero-sum trade-offs are only expected under the highly implausible assumptions that responses are entirely independent of stimulus properties (bottom row of Figure \@ref(fig:AA-result-changes-in-decision-making)).
