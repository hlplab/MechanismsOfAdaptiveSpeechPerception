...
get_accuracy_from_updated_representations <- function(
  ...
  history.optimization_representations <<- #TO DO: history.optimization_representations does not get saved and check why?
    bind_rows(
...
update_bias_and_categorize_test <- function(
  ...
  print(paste0("response.mean.se is",u.test$response.mean.se))
  print(paste0("u test n sims is", u.test$n.sims_so_far))

  if (verbose) # TO D0: check here why setting verbose = T when calling this function does not print out the u.test
    u.test %>%
    print()
...
# 
# bias.pred7 <- d.AA.exposure %>%
#         nest(data = -c(Condition, Subject)) %>%
#         crossing(
#           posterior.lapse_rate = c(0, 1),
#           beta_pi = c(0, 0.1, 1)) %>% # specify what parameters to plot
#         crossing(
#           m.ia.VOT_f0.AA %>%
...
#                                                      beta_pi = .x$beta_pi,
#                                                      exposure = .x$data[[1]],
#                                                      test = d.AA.test2 %>% filter(Condition == "L2-accented")), .keep = TRUE, verbose = T)
...
# make_results_plot_decision_making(data = bias.pred8, data_best = best_performing_parameters.bias)
```
...
construct_three_models <- function(
  ...
  # representations.res <- run_model.representations.AA()
  # bias.res <- run_model.bias.AA()
  # normalization.res <- run_model.normalization.AA()
  # 
  # representations.pred <- representations.res$visualization
  # bias.pred <- bias.res$visualization
  # normalization.pred <- normalization.res$visualization
  ...
  p.AA.representations <- make_results_plot_representations(data = representations.pred, data_best = representations.res$best)
  ...
  p.AA.normalization <- make_results_plot_normalization(normalization.pred, normalization.res$best)
...
# select_three_best_models <- function(d, example_label = "Cue_reweighting"){
#   data_best1 <- readRDS(get_path(paste0("../models/best_performing_parameters.representations_", example_label,".rds")))
#   data_best2 <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
#   data_best3 <- readRDS(get_path(paste0("../models/best_performing_parameters.normalization_", example_label,".rds")))
#   
#   d.best <- bind_rows(
#     d %>%
#       filter(ModelType == "representations") %>%
#       filter(category == Item.Intended_category & prior_kappa %in% prior_kappa[which.min(abs(as.numeric(as.character(prior_kappa)) - data_best1$par[1]))] & #select the parameters (within the plotted range) that are closest to the best performing parameterization
#                   prior_nu %in% prior_nu[which.min(abs(as.numeric(as.character(prior_nu)) - data_best1$par[2]))]),
#     d %>% 
#       filter(ModelType == "decision-making") %>%
#       filter(category == Item.Intended_category & posterior.lapse_rate %in% posterior.lapse_rate[which.min(abs(posterior.lapse_rate - data_best2$par[1]))] & 
#                   beta_pi %in% beta_pi[which.min(abs(beta_pi - data_best2$par[2]))]),
#     d %>%
#       filter(ModelType == "normalization") %>%
#       filter(category == Item.Intended_category & prior_kappa.normalization %in% prior_kappa.normalization[which.min(abs(as.numeric(as.character(prior_kappa.normalization)) - data_best3$par[1]))])) %>%
#     mutate(ModelType = factor(ModelType, levels = c("representations", "decision-making", "normalization"))) %>%
...
cases = c("Contrast_shift", "Contrast_reduction", "Contrast_collapse")
...
d.AA.exposure.target.cases <- rbind(
  ...
  mutate(Case = factor(Case, levels = c("Contrast shift", "Contrast reduction", "Contrast collapse")))
