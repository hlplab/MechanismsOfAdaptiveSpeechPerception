...
```{r AA-table-models-parameter-setting, fig.width=base.width*3 + .5, fig.height=base.height + .5, fig.cap="(ref:study-AA-exposure-test-plot)", warning=FALSE}
...
d.parameters <- data.frame(c("Changes in representations", "Changes in representations", "Changes in decision-making", "Changes in decision-making", "Changes in normalization"),
  c("$\\kappa_{c,0}$", "$\\nu_{c,0}$", "$\\lambda$", "$\\pi_c$", "$\\kappa_0$"),
                           c(c(paste0(range.prior_kappa[1], ", " ,range.prior_kappa[2])),
                           c(paste0(range.prior_nu[1], ", " ,range.prior_nu[2])),
                           c(paste0(range.lapse_rate[1], ", " ,range.lapse_rate[2])),
                           c(paste0(range.beta_pi[1], ", " ,range.beta_pi[2])),
...
kable(d.parameters %>% collapse_rows_df(Model), booktabs = TRUE) %>%
  collapse_rows(1, latex_hline = "major", valign = "middle") %>% 
#  row_spec(5, hline_after = T) %>% TO-DO: fix the formatting of the table
  kable_styling()
...
# run_model.representations.AA <- function(){
#    if (RESET_MODELS || !file.exists(paste0("../models/best_performing_parameters.representations_", example_label,".rds"))) {
#     best_performing_parameters.representations <-
#     optim(
#       # Parameters are kappa, nu
...
#     }
#   
#   representations.res <- list("best" =  best_performing_parameters.representations, "visualization" = representations.pred)
#   return(representations.res)
# }
# 
# d.AA.representations <- run_model.representations.AA()$visualization
# best_performing_parameters.representations <- run_model.representations.AA()$best
```
...
update_bias_and_categorize_test <- function(
  ...
  cues = c("VOT", "f0_Mel"),
  control = list(
    min.simulations = 5, #5 #DEBUG
    max.simulations = 10, #10
    step.simulations = 1, #1
    target_accuracy_se = .005), #.005
  add_updates = NULL,
  verbose = FALSE
) {
  u <- 
  ...
  # print(paste0("response.mean.se is",u.test$response.mean.se)) #DEBUG
  # print(paste0("u test n sims is", u.test$n.sims_so_far))
 ...
 if (control[["min.simulations"]] < control[["max.simulations"]] & (u.test %>%
   ...
   # print("part1") #DEBUG
   # print(paste0("u.test is", u.test %>% pull(response.mean.se)))
   #
 ...
 #  else {
 #   #print("routing else") #DEBUG
 # # # Get average predicted accuracy (and its SE) for each item across all simulations
 #   
 #   # u %<>%
 #   #   group_by(!!! syms(setdiff(names(.), c("sim","posterior","response")))) %>%
 #   #   mutate( #DEBUG:change summarise to mutate
...
get_average_bias_results_across_simulations <- function(data){
  output <- data %>%
    group_by(!!! syms(setdiff(names(.), c("sim","posterior","response")))) %>%
    summarise( #DEBUG:change summarise to mutate
      response.n_sims = n_distinct(sim),
      response.se = sd(response) / sqrt(response.n_sims),
      response = mean(response)) %>%
    relocate(
...
get_accuracy_from_updated_bias <- function(
  ...
  # print(paste0("current lapse rate is", lapse_rate)) #DEBUG
  # print(paste0("current beta pi is", beta_pi)) #DEBUG
  # 
  u <-
    update_bias_and_categorize_test(
    prior = prior,
    lapse_rate = lapse_rate, #DEBUG: this is not necessary for the current calling of update_bias_categorize_test because the lapse_rate in prior has been assigned the value of the lapse_rate used in the current optimization search (i.e., pars[1])
    beta_pi = beta_pi,
    exposure = exposure,
    test = test,
    verbose = verbose,
    control = list(
      ...
      max.simulations = 12, #100 #DEBUG
      step.simulations = 2, #2 #DEBUG
      target_accuracy_se = .002)) # .002 #DEBUG
...
history.optimization_bias <<-
  ...
  # 
  # print(paste0("current lapse rate after updating is", lapse_rate)) #DEBUG
  # print(paste0("current beta pi after updating is", beta_pi)) #DEBUG
  # print(paste0("current accuracy after updating is", u)) #DEBUG
...
### DEBUG
#temp <- get_accuracy_from_updated_bias(pars = c(0.75, 0.1))

# d.AA.test2 <- make_accent_adaptation_test_design(
#  # d.AA.exposure, 
#   experimenter.ideal_observer = m.io.VOT_f0.AA, shift_mean_1 = 0, shift_mean_2 = 0)
# 
# make_exposure_test_plot(exposure.data = d.AA.exposure, test.data = d.AA.test2)
# 
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
    ...
    best_performing_parameters.bias <-
      optim(
        ...
        control = list(
          fnscale = -1, # DEBUG:turns the optimization problem into a maximization problem (i.e., maximizing accuracy)
          factr = 10^8)) # DEBUG:controls the convergence of the "L-BFGS-B" method
...
# run_model.bias.AA <- function(){
#   
#   find_best_model.bias.AA()
#   plot_sample_model.bias.AA()
#   
#   bias.res <- list("best" =  best_performing_parameters.bias, "visualization" = bias.pred)
#   return(bias.res)
# }

# d.AA.bias <- run_model.bias.AA()$visualization
# best_performing_parameters.bias <- run_model.bias.AA()$best

posterior.lapse_rate.plot = c(0, 0.005, 0.05, 0.5, 1) # DEBUG change
...
## DEBUG
# best_performing_parameters.bias.debug1 <- find_best_model.bias.AA()
# temp <- readRDS(get_path("../models/nov27/best_performing_parameters.bias_Cue_reweighting.rds"))

# d.AA.bias2 <- plot_sample_model.bias.AA(
#   lapse_rate = best_performing_parameters.bias$par[1],
#   beta_pi = best_performing_parameters.bias$par[2]
# )
# 
# bias.pred8.no.transformation <- d.AA.exposure %>%
#         nest(data = -c(Condition, Subject)) %>%
#         crossing(
...
```{r AA-result-changes-in-decision-making-debug, eval = FALSE}
...
# run_model.normalization.AA <- function(){
#   if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.normalization_", example_label,".rds")))) {
#     best_performing_parameters.normalization <-
#     optim(
#       par = log(mean(range.prior_kappa.normalization)),
...
#      normalization.pred <- readRDS(get_path(paste0("../models/d.AA.results.normalization_", example_label,".rds")))
#    }
#   normalization.res <- list("best" =  best_performing_parameters.normalization, "visualization" = normalization.pred)
#   return(normalization.res)
# }
# 
# d.AA.normalization <- run_model.normalization.AA()$visualization
# best_performing_parameters.normalization <- run_model.normalization.AA()$best
...
construct_three_models <- function(
  ...
  ggsave(paste0('../figures/plotly/AA_cases/exposure_test_distribution_', example_label, '.png'), plot = p.exposure_test, width = base.width*3 + .5, height = base.height + .5, dpi = 300)
  ...
  p.AA.representations <- make_results_plot_representations(data = representations.pred, data_best = best_performing_parameters.representations)
  ggsave(paste0('../figures/plotly/AA_cases/representations_', example_label, '.png'), plot = p.AA.representations, width = 2*(base.width*length(levels(factor(representations.pred$prior_kappa))) + .5), height = 2*(base.height*length(levels(factor(representations.pred$prior_nu))) + .5), dpi = 300)

  p.AA.bias <- make_results_plot_decision_making(data = bias.pred, data_best = best_performing_parameters.bias)
  ggsave(paste0('../figures/plotly/AA_cases/bias_', example_label, '.png'), plot = p.AA.bias, width = 2*(base.width*length(levels(factor(bias.pred$beta_pi))) + .5), height = 2*(base.height*length(levels(factor(bias.pred$posterior.lapse_rate))) + .5), dpi = 300)

  p.AA.normalization <- make_results_plot_normalization(normalization.pred, best_performing_parameters.normalization)
  ggsave(paste0('../figures/plotly/AA_cases/normalization_', example_label, '.png'), plot = p.AA.normalization, width = 2*(base.width*length(levels(factor(normalization.pred$prior_kappa.normalization))) + .5), height = 2*(base.height + .5), dpi = 300)
...
# d.AA.results.contrast_shift.best <- select_three_best_models(d.AA.results.contrast_shift, example_label = "Contrast_shift")
```
...
# d.AA.results.contrast_reduction.best <- select_three_best_models(d.AA.results.contrast_reduction, example_label = "Contrast_reduction")
```
...
shift.cue2 <- 0
var.ratio <- 1
example_label = "Contrast_collapse" #COME BACK HERE AND FIX IT FOR get_parameters_phonetic_contrast.AA
...
# d.AA.results.contrast_collapse.best <- select_three_best_models(d.AA.results.contrast_collapse, example_label = "Contrast_collapse")
# rm(example_label) # CHANGE this when example_label called by get_parameters_phonetic_contrast.AA is fixed
#DEBUG
# history.optimization.bias.yes <- readRDS("../models/d.AA.history.optimization/d.AA.history.optimization.bias_Contrast_collapse.rds")
# d.AA.bias.yes <- readRDS("../models/temp_here/d.AA.results.bias_Contrast_collapse.rds")
# d.AA.bias.now <- get_average_bias_results_across_simulations(d.AA.results.contrast_collapse.best)     
# best_performing_parameters.bias.yes <- readRDS("../models/temp_here/best_performing_parameters.bias_Contrast_collapse.rds")
# best_performing_parameters.bias.now <- readRDS("../models/best_performing_parameters.bias_Contrast_collapse.rds")
```
...
for (i in 1:length(cases)){
  ...
  history.optimization.representations.current <- readRDS(get_path(paste0("../models/d.AA.history.optimization.representations_", example_label,".rds"))) %>%
    mutate(Case = example_label) 
  history.optimization.representations.all <- bind_rows(history.optimization.representations.all, history.optimization.representations.current)
 rm(history.optimization.representations.current)
 
  #load history of optimization for bias model
  history.optimization.bias.current <- readRDS(get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds"))) %>%
    mutate(Case = example_label) 
  history.optimization.bias.all <- bind_rows(history.optimization.bias.all, history.optimization.bias.current)
  rm(history.optimization.bias.current)
  
  #load history of optimization for normalization model
  history.optimization.normalization.current <- readRDS(get_path(paste0("../models/d.AA.history.optimization.normalization_", example_label,".rds")))
  
  history.optimization.normalization.all <- bind_rows(history.optimization.normalization.all, history.optimization.normalization.current)
...
example_label = "Contrast_reduction" # COME BACK HERE TO FIX THE ASSIGNMENT OF EXAMPLE_LABEL
...
In summary, benefits of L2-accented exposure expected under each change mechanism hinges on the specifics of L2-accented categories. Beyond the accent scenarios examined here (cue-reweighting, contrast reduction, contrast shift, and contrast collapse), naturally produced L2 accents exhibit substantial variations in complex ways, which can pose difficulties to L1-listeners and researchers alike. At the same time, they also offer a unique and fertile ground to test the relative engagement of each change mechanism. Because the three change mechanisms would predict distinct perceptual outcomes depending on the properties of the exposure and test stimuli, these differences can be used to distinguish between model predictions. In the general discussion below, we explore how such tests are possible.
...
parameters.results.best.cases <- bind_rows(best_performing_parameters.representations.par,                      best_performing_parameters.bias.par, best_performing_parameters.normalization.par) %>%
  ...
  mutate(parameterization = case_when(
   ModelType == "representations" ~ paste0("kappa = ", round(prior_kappa, digits = 0), ", nu = ", round(prior_nu, digits = 0)),
   ModelType == "decision-making" ~ paste0("lambda = ", round(lapse_rate, digits = 2),   ", beta = ", round(beta_pi, digits = 2)),
   ModelType == "normalization" ~ paste0("kappa = ", round(prior_kappa, digits = 0)))) %>%
   mutate(
      ModelType = factor(paste0("Changes in ", ModelType), levels = c("Changes in representations", "Changes in decision-making", "Changes in normalization"))) %>%
...
knitr::kable(parameters.results.best.cases, caption = "Best-performing parameters across different accent scenarios", align = 'l', booktabs = TRUE) %>%
