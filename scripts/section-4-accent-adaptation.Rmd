...
update_bias_and_categorize_test <- function(
 ...
 # # Get average predicted accuracy (and its SE) for each item across all simulations
   u.bySim <<- u %>%
     mutate(Condition = .env$Condition)
  # saveRDS(u.bySim, get_path(paste0("../models/d.AA.bias.bySim.rds")))
...
find_best_model.bias.AA <- function(){
  ...
  # temp1  <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,"_old.rds"))) #DEBUG
  # temp2  <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
  # temp1$par == temp2$par
  #  temp3 <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
...
# make_results_plot_decision_making(data = bias.pred.ver1, data_best = best_performing_parameters.bias)
```
...
get_accuracy_from_updated_normalization <- function(
  ...
  history.optimization_normalization <<- 
    bind_rows(
      history.optimization_normalization, 
      tibble(kappa.normalization = kappa.normalization, accuracy = u, example_label = .env$example_label)) #DEBUG remove the recording of example_label
