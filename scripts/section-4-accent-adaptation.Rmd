...
Figure \@ref(fig:AA-result-changes-in-decision-making) shows the effects of changes in decision-making as a function of the rate at which response biases change $\beta_{\pi}$ and the lapse rate $\lambda$, averaged across all simulations for each parameterization. As in Case Study 1, the figure shows the average across repeated simulations, each time randomizing the order of exposure. For Case Study 2, we conducted up to a maximum of 50 simulations (since larger $\beta_{\pi}$ exacerbate the order sensitivity of the model) or until the standard error of the accuracy in the L2-accented condition was less than .005 (across the simulations). For smaller $\beta_{\pi}$ like those employed in Case Study 1, the standard error criterion was always reached within $<XXX$ simulations. <!-- TO DO: can you fill this in? --> For the $\beta_{\pi} > XXX$, this criterion was not reached even after 50 simulations. <!-- TO DO: can you fill this in? --> These $\beta_{\pi}$ are thus implausible hypotheses for a model of accent adaptation, since the order of exposure stimuli has generally not been found to have a large impact on subsequent perception [at least for the amount of exposure typically considered in experiments on adaptive speech perception, @REF]. <!-- TO DO: please fill in 2-3 REFs -->
