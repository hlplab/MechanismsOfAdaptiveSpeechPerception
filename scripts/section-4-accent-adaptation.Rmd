...
# Is there a change in category variance? If so, multiple category variance by a constant; var.ratio = 1 means no change in category variance
#var.ratio = 1
var.ratio.cue1 = 1.4
var.ratio.cue2 = 0.6
...
get_parameters_phonetic_contrast.AA <- function(ideal.observer,
   ...
   Sigma_nonnative_B <- matrix(c(Sigma_native_B[1]*var.ratio.cue1, Sigma_native_B[2], Sigma_native_B[3], Sigma_native_B[4]*var.ratio.cue2), nrow=2)
   ...
   # Sigma_nonnative_A <- cor2cov(Omega_nonnative_A, sqrt(diag(Sigma_native_A))*var.ratio)
    Sigma_nonnative_A <- matrix(c(Sigma_native_A[1]*var.ratio.cue1, Sigma_native_A[2], Sigma_native_A[3], Sigma_native_A[4]*var.ratio.cue2), nrow=2)
...
p.AA.bias <- make_results_plot_decision_making(d.AA.bias, best_performing_case.bias)
  ggsave(paste0('../figures/plotly/temp/', '_bias.png'), plot = p.AA.bias, width = 2*(base.width*length(levels(factor(d.AA.bias$beta_pi))) + .5), height = 2*(base.height*length(levels(factor(d.AA.bias$posterior.lapse_rate))) + .5), dpi = 300)
...
make_results_plot_normalization(d.AA.normalization %>% filter(normalization == "centered based\non exposure"), best_performing_case.normalization) # only show the scenarios when normalization is applied, best_performing_case.normalization

p.AA.normalization <- make_results_plot_normalization(d.AA.normalization %>% filter(normalization == "centered based\non exposure"), best_performing_case.normalization)
ggsave(paste0('../figures/plotly/temp/', '_normalization.png'), plot = p.AA.normalization, width = 2*(base.width*length(levels(factor(d.AA.normalization$prior_kappa.normalization))) + .5), height = 2*(base.height + .5), dpi = 300)
```
