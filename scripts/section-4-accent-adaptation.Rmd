...
d.AA.representations <-
  ...
  left_join(
    d.AA.test %>%
      distinct(x, Item.Intended_category), by = "x")

d.AA.representations2 <- d.AA.exposure %>%
...
d.AA.representations3 <- d.AA.representations2 %>%
  mutate_at(
    vars(starts_with("prior_")),
    ~ factor(as.character(.x), levels = as.character(rev(sort(unique(.x))))))
...
make_results_plot_representations(data = d.AA.representations2, data_best = best_performing_parameters.representations)
```
