...
get_accuracy_from_updated_representations <- function(
    ...
    exposure = d.AA.exposure %>% filter(Condition == "L2-accented"), 
    test = d.AA.test %>% filter(Condition == "/d/_L2-accented")
) {
  suppressWarnings(
    update_NIW_ideal_adaptor_incrementally(
      prior = model %>% 
        droplevels() %>%
        lift_MVG_ideal_observer_to_NIW_ideal_adaptor(kappa = exp(log_kappa), nu = exp(log_nu)),
      exposure = exposure,
      ...
      add_test_tokens(test) %>%
      mutate(posterior.categorization = map(posterior, ~ get_categorization_function_from_NIW_ideal_adaptor(.x, logit = TRUE))) %>%
      add_categorization() %>%
...
best_performing_parameters.representations <- 
  optim(
    ...
    control = list(
      ...
      factr = 10^11)) # larger factr means higher optimum but slower convergence
```

(ref:AA-result-changes-in-representations) Predictions of a learning model that derives accent adaptation as changes in category representations. Predicted categorization accuracies for the L2-accented test tokens after `r conditions.AA[1]` and `r conditions.AA[2]` exposure, as a function of the strength of the prior beliefs in category means ($\kappa_{0,c}$) and covariances ($\nu_{0,c}$). The average accuracy across /d/ and /t/ is shown above the bars for each exposure condition. Error bars show 95% bootstrapped confidence intervals. The highlighted panel is the one closest to the best-performing parameterization ($\kappa_{0,c} = `r exp(best_performing_parameters.representations$par[1])`$; $\nu_{0,c}=`r exp(best_performing_parameters.representations$par[2])`$; overall accuracy $=`r best_performing_parameters.representations$value`$).
      ...
      geom_rect(
        data = data %>%
          ...
          mutate(
            highlight = 
              ifelse(
                prior_kappa == prior_kappa[which.min(abs(prior_kappa - data_best$par[1]))] & 
                  prior_nu == prior_nu[which.min(abs(prior_nu - data_best$par[2]))],
                T, F)),
...
make_results_plot_representations(d.AA.representations, best_performing_parameters.representations)
```
...
best_performing_parameters.bias <- 
  optim(
    # Parameters are lapse_rate, beta_pi
    par = c(0, log(.1)), 
    ...
    insert_layers(
      geom_rect(
        data = data %>%
          ...
          mutate(highlight = 
                   ifelse(
                     posterior.lapse_rate == posterior.lapse_rate[which.min(abs(posterior.lapse_rate - data_best$par[1]))] & 
                       beta_pi == beta_pi[which.min(abs(beta_pi - data_best$par[2]))],
                     T, F)),
        aes(fill = highlight), alpha = .1, inherit.aes = F,
        xmin = -Inf,xmax = Inf, ymin = -Inf,ymax = Inf)
    ) +
...
make_results_plot_decision_making(d.AA.bias, best_performing_parameterization.bias)
```
    ...
    insert_layers(
      geom_rect(
        data = 
          ...
          mutate(highlight = 
                   ifelse(
                     prior_kappa.normalization == prior_kappa.normalization[which.min(abs(prior_kappa.normalization - data_best$par[1]))],
                     T, F)),
        aes(fill = highlight), alpha = .1, inherit.aes = F,
        xmin = -Inf,xmax = Inf, ymin = -Inf,ymax = Inf)) +
    scale_fill_manual(breaks = c(T, F), values = c("darkgray", "white")) +
