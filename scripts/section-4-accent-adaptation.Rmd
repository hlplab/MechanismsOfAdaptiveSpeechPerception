...
plot_sample_model.bias.AA <- function(
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/d.AA.results.bias_", example_label,".rds")))) {
    bias.pred <- d.AA.exposure %>%
      ...
      group_modify(~ update_bias_and_categorize_test(
        prior = .x$prior[[1]],
...
Figure \@ref(fig:AA-result-changes-in-decision-making) shows the effects of changes in decision-making as a function of the rate at which response biases change $\beta_{\pi}$ and the lapse rate $\lambda$, averaged across all simulations for each parameterization. As in Case Study 1, the figure is based on repeated simulations, each time randomizing the order of exposure. For Case Study 2, we used more (XXX) simulations per panel since larger $\beta_{\pi}s$ exacerbate the order-sensitivity of this change model. The algorithm used to determine the best-performing model dynamically determined the number of necessary simulations. <!-- TO DO: Xin, can you fill in? -->
