...
## Results
Paralleling Case Study 1, we ask which of the three change models can account for the signature results of accent adaptation experiments. Specifically, we assess for each change model whether it can explain the two types of signature results observed in, for example, Xie et al. [-@xie2016jep]: (1) for the control L1-accented exposure, /t/ test tokens should be recognized with higher accuracy than /d/ test tokens, since only the distribution of the latter deviates from L1 listeners' expectations; and (2) L2-accented exposure should lead to a significant increase of recognition accuracy for /d/ test tokens without (equivalently) decreased accuracy for /t/ test tokens, resulting in increased accuracy overall. 
...
Importantly, Figure \@ref(fig:AA-result-changes-in-decision-making) also shows that changes in response biases do not necessarily result in the type of zero-sum trade-off---accuracy improving for one category at the cost of accuracy for the other category---that is sometimes attributed to them: L2-accented exposure can predict improvements in *overall* recognition accuracy , averaging over the recognition accuracies for /d/ and /t/ (e.g., top right panel). Indeed, zero-sum trade-offs are only expected under the highly implausible assumptions that responses are entirely independent of stimulus properties (bottom row of Figure \@ref(fig:AA-result-changes-in-decision-making)).
...
## Summary
Paralleling Case Study 1 on perceptual recalibration, we find that any of the three change mechanisms can qualitatively explain the signature results of experiments on accent adaptation. Contrary to common assumptions, even non-representational mechanisms---normalization and decision-making---can lead to _overall_ improvements in recognition accuracy, rather than only zero-sum trade-offs. Overall improvements in accuracy after exposure to an unfamiliar accent are thus _not_ diagnostic of representational changes. This supports the conclusion of Case Study 1: at the level of analysis that is commonly applied in previous work (and thus here), experiments on accent adaptation do not decisively rule out any of the three mechanisms as a possible cause for the facilitatory effects of exposure.

Beyond their qualitative fit to existing behavioral patterns, these three change models exhibit some quantitative differences. The maximal benefits of L2-accented exposure are predicted to be largest from the representational change model (overall improvements from `r best_performing_case$mAcc[1]` to `r best_performing_case$mAcc[2]`), followed by the decision-making change model (from `r best_performing_case.bias$mAcc[1]` to `r best_performing_case.bias$mAcc[2]`) and smallest in the normalization change model (`r best_performing_case.normalization$mAcc[1]` to `r best_performing_case.normalization$mAcc[2]`). <!--This difference has to be interpreted with caution for at least two reasons. First, the present simulations consider only a range of parameter settings. It is thus possible that, for example, higher values of $\beta_{\pi}$ yield larger benefits of L2-accented exposure from changes in decision-making. -->We note, however, that the magnitude of overall accuracy improvements after talker-specific L2-accented exposure that has been observed in experiments [e.g., about 6% in @xie2016jep; ~10% in @bradlow-bent2008] falls well within the range that any of the three change models can explain (for the present stimuli and parameter settings). 

Importantly, these differences do not mean that the representational change model is always more advantageous than the other two models. Rather, it exemplifies a critical prediction of the ASP framework: the specific quantitative benefit of L2-accented exposure predicted by each of the change mechanisms depends on the exposure and test stimuli of the experiment, and thus---if naturally produced speech is employed---the specific nature of the non-native characteristics of the L2 accent. To make this point more concrete, Figure \@ref(fig:AA-result-comparison-best-three-models-case2) presents three additional (simulated) scenarios that are common for L2 accents, along with the best-performing variants of each change model in each scenario. The top panel shows an example of *contrast shift*, for which the L2 accent maintains the strength of the L1 contrast but shifts both categories along one or more of the cue dimensions [e.g., characteristic of word-initial stops in French-accented English, @Sumner2011a]. The middle panel shows an example of *contrast reduction*, for which a contrast present in the L1 is reduced in the L2 accent, resulting in greater category overlap [e.g., characteristic of vowels in Spanish-accented English, @wade2007]. The bottom panel shows an example of *contrast collapse*, for which a contrast is no longer maintained in the L2 accent, resulting in a complete category overlap [e.g., characteristic of a loss of distinction between [s] and [`r linguisticsdown::cond_cmpl("θ")`] in Mandarin-accented English, @zheng-samuel2020]. Across these scenarios, the L1-accented exposure condition is held constant and identical to that in the case study above, whereas the L2-accented exposure condition is manipulated to exhibit different accent patterns (shown in the left panel). Everything else is identical to that in the case study above, including the test phase generation procedure and the model parameter settings.

We note a few key findings, based on the particular range of parameters considered here, and the stimulus statistics of the simulated experiments. First, the predicted difficulty for L1-English listeners for the two L2-accented categories varies across these scenarios, despite that the L1-accented exposure is held constant. For instance, the recognition of /t/ is more difficult than the recognition of /d/ in the case of a contrast shift (top panel), contrary to the two scenarios at the bottom and the main scenario simulated above. This pattern captures the fact that the realizations of the same target contrast form different acoustic-phonetic distributions across different L2 accents and may exhibit greater or lesser difficulty for L1-English listeners. Second, the overall benefit of L2-accented exposure, compared to L1-accented exposure, and critically the relative benefit of each change model, differ across these scenarios. Let us unpack this point a bit.

First, changes in decision making does not promote any benefit in the case of a pure contrast shift (top panel). This outcome aligns with the zero-sum trade-offs often attributed to this change mechanism. However, as the other simulated scenarios show, this zero-sum trade-off is not an inherent property of this mechanism, but rather a possible outcome given certain properties of the L2 accent (or L1-L2 accent difference). Furthermore, in this particular scenario, changes in normalization are predicted to as beneficial as changes in representations, both yielding large improvements in overall recognition accuracy. In contrast, in the case of a contrast reduction (middle panel)---unlike for the main scenario for Case Study 2 and the case of a contrast shift---the best overall performance is predicted from changes in decision making, followed by normalization, rather than changes in representations. For these two examples, there is again an overall benefit of L2-accented exposure, compared to L1-accented exposure. However, the predicted outcome of L2-accented exposure could also be the absence of such a benefit if that is what the category statistics support (e.g., if a category contrast present in the L1 accent is completely collapsed in the L2 accent). This result pattern is simulated and presented in the bottom panel. Therefore, an *absence of improvements* should not be taken as evidence to reject the hypothesis of representational changes. Such a conclusion is only valid if it is also shown that *successful learning of the exposure statistics* would be *predicted* to facilitate comprehension during test. Whether this prediction holds for a given experiment depends on the exposure and test stimuli---a fact that is sometimes not considered in the interpretation of experiments [cf. @zheng-samuel2020]. If an experiment employs exposure and test stimuli with distributional statistics similar to the case of a category collapse here, for instance, then a lack of behavioral improvement is actually compatible with a representational change account.

Together, these results highlight that any of the three change mechanisms can *potentially* explain even complex adaptive behavior, including adaptation to natural accents. Critically, however, changes in how L2-accented categories differ from their L1-accented counterparts for the same L1 contrast can significantly alter the relative predicted benefits of each change model. The best-fitting parameters for each change model are very similar across these three scenarios simulated and the main scenario presented above. This is a direct consequence of us having only considered a very specific subset of changes that an L2 accent can introduce. Specifically, none of the simulated scenarios considered here introduce changes in category variance or covariance. In natural accents, the specific manner in which L2-accented categories differ from their L1 counterparts depends on the sound systems of both languages, including differences in how the talkers' L1 and L2 weight the same cues or even whether a cue is used at all in the realization of a phonetic contrast [@ingvalson2011; @liu-holt2015; @schertz2013; @yamada-tohkura1992]. These differences cause L2-accented categories to differ from L1-accented categories not only in terms of their category means, but also in category variances, and/or category covariances [the category-specific correlation between cues, e.g., @smith2019; @wade2007; @vaughn2019; @xie-jaeger2020]. Across accents and languages, differences in these distributional properties offer a rich ground to test the relative engagement of each change mechanism, which is predicted to be dependent on the properties of the exposure and test stimuli. These properties, however, are rarely ever reported in the literature, and their predicted consequences for perception are even less commonly modeled [for an exception, see @tan2021]. Next, we discuss the consequences of our findings and provide recommendations for future research.
