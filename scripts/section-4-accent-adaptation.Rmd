...
step_n_simulations_for_bias_update <- 1
...
update_bias_and_categorize_test <- function(
  ...
  control = list(
    ...
    increment = step_n_simulations_for_bias_update,
    target_se = target_se_for_bias_update),
  add_updates = NULL,
  verbose = FALSE
) {
  u <- 
  ...
  if (!is.null(add_updates)) u %<>% bind_rows(add_updates)
    ...
    if(u %>% 
      ...
      u <-
        update_bias_and_categorize_test(
          ...
          cues = cues,
          control = list(
            min.simulations = control[["increment"]], 
            max.simulations = control[["max.simulations"]] - control[["min.simulation"]], 
            increment = control[["increment"]],
            target_se = control[["target_se"]]),
          add_updates = u) 
      }
  ...
  u %>% 
    ...
    summarise(
      ...
      response.se = sd(response) / sqrt(n_distinct(response, na.rm = T)))
}
...
# TO DO: add if statement here: if RESET_MODELS == T or if file does not exist, then
# run and store in models/
# else load rds into best_performing_parameters.bias
