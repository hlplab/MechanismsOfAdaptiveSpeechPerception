...
# Case 2 -- contrast reduction: no cue-weighting, but greater category overlap due to reduced distance (e.g., e.g., vowels in Spanish-accented English)
# min_VOT = 5
# dist.L2_category.cue1 <- 0.5
...
# Case 3 -- contrast shift: no cue-weighting, no change in category overlap, but a general shift along the primary cue dimension (e.g., word-initial stops in French-accented English)
# min_VOT = -100
...
# Case 4 -- contrast collapse: two categories are collapsed into one (i.e., here implemented as having the same category mean, e.g., s-th in Mandarin-accented English)
# min_VOT = -100
...
# Case 5 -- contrast shift + variance increase 
# min_VOT = -100
# dist.L2_category.cue1 <- 1
# dist.L2_category.cue2 <- 0
# shift.cue1 <- -1.2
...
As in Case Study 1, we first model changes in representations, and then compare them against change models in decision-making and normalization. In  all cases, we grid search a larger range of parameterizations than those in Case Study 1 in order to understand the full range of model behaviors as a function of them. <!--Specifically, for the representational change model, we start at the fastest learning rate for both category means and covariances ($\kappa_{0,c}$ = $\nu_{0,c}$ = 4) and stop the search at a slow-enough learning rate where no difference is predicted between exposure conditions. For the decision-making change model, $\lambda_{posterior}$ is varied between 0 and 1; for $\beta_{\pi}$, the search starts at 0 and stops when the overall recognition accuracy of L2-accented exposure condition plateaus. For the normalization change model, we start at the fastest learning rate ($\kappa_0$ = 1). -->The best-performing parameterizations presented in subsequent sections are the parameter settings that yield the highest overall recognition accuracy for the L2-accented exposure condition for each model. If multiple parameter settings yield equivalent predictions, then the paratemeterization with the slowest model updating rate is deemed as the 'best-performing' setting.
...
best_performing_case.representations.both <- d.AA.representations %>%
  filter(category == Item.Intended_category) %>%
  filter(prior_kappa == best_performing_case.representations$prior_kappa & prior_nu == best_performing_case.representations$prior_nu) %>%
  group_by(Condition, Subject, prior_kappa, prior_nu) %>%
  summarise(mAcc = mean(response)) 
```
  ...
  insert_layers(
    geom_rect(data = data %>%
                ...
                mutate(highlight = case_when(
                  prior_kappa == data_best$prior_kappa & prior_nu == data_best$prior_nu ~ T,
                  T ~ F)),
...
make_results_plot_representations(d.AA.representations, best_performing_case.representations)
```
...
```{r study-AA-models-changes-in-decision-making}
...
best_performing_case.bias.both <- d.AA.bias %>%
  filter(category == Item.Intended_category) %>%
  filter(posterior.lapse_rate == best_performing_case.bias$posterior.lapse_rate & beta_pi == best_performing_case.bias$beta_pi) %>%
  group_by(Condition, Subject, posterior.lapse_rate, beta_pi) %>%
  summarise(mAcc = mean(response)) 
```
...
p.AA.results.bias +
  ...
  insert_layers(
    ...
    filter(category == Item.Intended_category) %>%
                  ...
                  mutate(highlight = case_when(
                    (posterior.lapse_rate == data_best$posterior.lapse_rate & beta_pi == data_best$beta_pi) ~ T,
                    T ~ F)),
                aes(fill = highlight), alpha = .1, inherit.aes = F,
                xmin = -Inf,xmax = Inf, ymin = -Inf,ymax = Inf)
      ) +
...
make_results_plot_decision_making(d.AA.bias, best_performing_case.bias)
```
...
best_performing_case.normalization.both <- d.AA.normalization %>%
  filter(category == Item.Intended_category) %>%
  filter(normalization == "centered based\non exposure") %>%
  filter(prior_kappa.normalization == best_performing_case.normalization$prior_kappa.normalization) %>%
  group_by(Condition, Subject, prior_kappa.normalization) %>%
  summarise(mAcc = mean(response)) 
```
...
make_results_plot_normalization <- function(data, data_best){
  ...
  filter(category == Item.Intended_category) %>% # show the response for intended category
  filter(normalization == "centered based\non exposure"), 
  aes(x = Condition, y = response, fill = Item.Intended_category, alpha = Condition)) +
  stat_summary(fun = mean,
               geom="bar", position = pos,
               width = 0.6) +
  ...
  p.AA.results.normalization +
    ...
    insert_layers(
        geom_rect(data = data %>%
                    ...
                    mutate(highlight = case_when(
                      (prior_kappa.normalization == data_best$prior_kappa.normalization) ~ T,
                      T ~ F)),
                  aes(fill = highlight), alpha = .1, inherit.aes = F,
                  xmin = -Inf,xmax = Inf, ymin = -Inf,ymax = Inf)
        ) +
...
make_results_plot_normalization(d.AA.normalization, best_performing_case.normalization)
```
...
To make this point more concrete, Figure \@ref(fig:AA-result-comparison-best-three-models-cases) presents three additional (simulated) scenarios that are common for L2 accents, along with the best-performing variants of each change model in each scenario. The top panel shows an example of *contrast shift*, where two categories in L2-accented speech are shifted along one or more of the cue dimensions as compared to L1-accented speech [e.g., attested for word-initial stops in French-accented English, @Sumner2011a]. The middle panel shows an example of *contrast reduction*, where an L2-accent shows a greater category overlap than an L1-accent [e.g., attested for vowels in Spanish-accented English, @wade2007]. The bottom panel shows an example of *contrast collapse*, where two contrasting categories in an L1-accent overlap substantilly or completely in an L2-accent [e.g., characteristic of a loss of distinction between [s] and [`r linguisticsdown::cond_cmpl("θ")`] in Mandarin-accented English, @zheng-samuel2020]. The three scenarios are created by merely varying category means along VOT for the L2-accented categories, without introducing more complex variation in category variance and covariance. The L1-accented exposure condition is the same as in the scenario above (Figure \@ref(fig:study-AA-exposure-test-plot) and held constant across these three scenarios. As above, test tokens are randomly sampled from the distribution of each of the L2-accent category. Consequently, test tokens varied across the three scenarios examined here. For consistency, we focus on the model parameter settings identical to the main scenario. 

Contrasted side-by-side, the three scenarios differ significantly both in terms of the predicted levels of difficulty in the L2-accent category recognition for L1-listeners (L1-accented exposure condition) and the amount of improvements expected post exposure (L2-accented exposure condition). For instance, the recognition of /t/ is more difficult than the recognition of /d/ in this specific case of a contrast shift (top panel), whereas it was the opposite in the other two scenarios as well as in the main scenario (Figures \@ref(fig:AA-result-changes-in-representations)-<!--\@ref(fig:AA-result-changes-in-decision-making),-->\@ref(fig:AA-result-changes-in-normalization)). This illustrates that arguably simple differences in the underlying distributional structures of L1- vs. L2-accent categories (e.g., contrast shift vs. reduction) could give rise to starkly different perceptual consequences for L1-accented listeners. Similarly, the relative benefit of L2-accented exposure---compared to L1-accented exposure---as predicted by each change model, differs across these scenarios. This latter point offers three insights of critical importance.

First, the same exact change model can support more or less benefit according to properties of an L2-accent (or L1-L2 accent difference). Consider the case of a contrast shift (top panel). While changes in representations and normalization both predict large improvements in overall recognition accuracy in the L2-accented exposure, changes in decision making does not (i.e., the predicted accuracy is the same for both the L1-accented and L2-accented exposure conditions). This outcome aligns with the zero-sum trade-offs often attributed to this change mechanism (i.e., The recognition of /t/ improves at the cost of the recognition of /d/). The heterogeneity of the results across the three scenarios makes it clear that the trade-off is not an inherent property of the decision-making mechanism. It is instead a possible outcome arising from a combination of a specific change model and properties of the L2 accent (or L1-L2 accent difference). 

Second, as mentioned above, changes of representations do not always outperform the other two mechanisms. In the case of contrast reduction (middle panel), the best overall performance is predicted from changes in decision making, followed by normalization, and representations. Surely it is possible that the representational model can perform better when a wider variety of parameters are considered. Nonethelesss, it is important to keep it in mind that the computational complexity afforded for the representational change model does not necessarily predict a greater degree of recognition improvements than the other two mechanisms.
...
In summary, benefits of L2-accent exposure expected under each change mechanism hinges on specifics of L2-accented categories.<!--The best-fitting parameters for each change model are very similar across these three scenarios simulated and the main scenario presented above. This is a direct consequence of us having-->Beyond the three scenarios examined here (contrast shift, reduction, and collapse), naturally produced L2-accents exhibit substantial variations. Some of these variations are random (e.g., individual variations) while others are relatively predictable (e.g., mismatches between L1- vs. L2 sound systems or L2 proficiency). However, even with more predictable sources of variations, there are a wide range of possibilities in terms of how a given L2-talker's L1 and L2 weight the same cues in signaling a specific phonetic contrast, or even whether a cue is used at all in the realization of a phonetic contrast [@ingvalson2011; @liu-holt2015; @schertz2013; @yamada-tohkura1992]. These differences cause L2-accented categories to deviate from L1-accented categories not only in terms of their means, but also in variances, and/or covariances [the category-specific correlation between cues, e.g., @smith2019; @wade2007; @vaughn2019; @xie-jaeger2020]. These differences can pose difficulties to L1-listeners and researchers alike. At the same time, they also offer a furtile ground to test the relative engagement of each change mechanism. Because the change mechanisms would predict different perceptual outcomes depending on the properties of the exposure and test stimuli, we can use different accent categories as testing grounds for distinguishing between distinct model predictions. In the general discussion below, we explore how such tests are possible.
...
d.AA.exposure.target.contrast.reduction <- make_accent_adaptation_exposure_design(
...
min_VOT = -100
var.ratio = 1
d.AA.exposure.target.contrast.collapse <-  make_accent_adaptation_exposure_design(
  experimenter.ideal_observer = m.io.VOT_f0.AA, 
...
min_VOT = -100
var.ratio = 1
d.AA.exposure.target.contrast.shift <- make_accent_adaptation_exposure_design(
  experimenter.ideal_observer = m.io.VOT_f0.AA, 
...
```{r AA-plot-results-additional-cases, fig.width=base.width*5 + .5, fig.height=base.height*3 + .5, fig.cap = "(ref:AA-plot-results-additional-cases)", warning=FALSE}
d.AA.results.best.cases <- rbind(
  ...
  mutate(
    Case = factor(Case, levels = c("Contrast reduction", "Contrast collapse", "Contrast shift"
                                   #, "Shift + var increase"
                                   ))) %>%
...
prow = plot_grid(p.AA.exposure, p.results.best.cases + theme(legend.position="none"), 
          labels = c('A)', 'B)'), 
          ncol = 2, 
