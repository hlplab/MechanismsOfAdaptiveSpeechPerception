...
As noted above, we have focused here on each change model's ability to predict the qualitative results typically observed in experiments on accent adaptation. The relative benefit of L2-accented exposure predicted by each of the change mechanisms depends on the exposure and test stimuli of the (simulated) experiment, and thus---if naturally produced speech is employed---the specific nature of the non-native changes introduced by the L2 accent. To make this point more concrete, Figure \@ref(fig:AA-result-comparison-best-three-models) presents the best-performing variants of each change model for a different (simulated) scenario that is common for L2 accents: the reduction of complete collapse of an L1 contrast. Compared to the scenario considered so far, the predicted performance in Figure \@ref(fig:AA-result-comparison-best-three-models)C shows a different pattern. Specifically, changes in decision-making are predicted to yield the greatest absolute benefits, followed by the normalization change model and then the representational change model. Counter-intuitive at first sight, the results again highlight that any of the three change mechanisms may explain even complex adaptive behavior, including adaptation to natural accents. Next, we discuss the consequences of these findings and provide recommendations for future research.
