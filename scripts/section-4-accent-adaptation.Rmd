...
run_model.bias.AA <- function(){
  ...
  bias.pred <- d.AA.exposure %>%
    nest(data = -c(Condition, Subject)) %>%
      crossing(
        lapse_rate = c(0, .05, 0.5, 1),
        beta_pi = c(0, .1, 0.5, 1, 4)) %>% # specify what parameters to plot
      crossing(
        m.ia.VOT_f0.AA %>%
          filter(prior_kappa == max(prior_kappa), prior_nu == max(prior_nu)) %>%
          nest(prior = everything())) %>%
      group_by(Condition, Subject, lapse_rate, beta_pi) %>%
    group_modify(~ update_bias_and_categorize_test(prior = .x$prior[[1]],
                                                   beta_pi = .x$beta_pi,
                                                   exposure = .x$data[[1]],
                                                   test = d.AA.test %>% filter(Condition == "L2-accented")), .keep = TRUE) %>%
    mutate_at(vars(starts_with("prior_")), ~factor(.x)) %>%
    mutate_at(vars(starts_with("prior_")), fct_rev) %>%
    ungroup()
  ...
  p.results +
    ...
    insert_layers(
      geom_rect(
        data = data %>%
          filter(category == Item.Intended_category) %>%
          distinct(lapse_rate, beta_pi) %>%
          ungroup() %>%
          mutate(
            highlight = 
              ifelse(
                lapse_rate %in% lapse_rate[which.min(abs(lapse_rate - data_best$par[1]))] & 
...
make_results_plot_decision_making(data = bias.pred, data_best = best_performing_parameters.bias)
```
...
construct_three_models <- function(
  ...
  d.AA.test <<- 
    make_accent_adaptation_test_design(
      d.AA.exposure, experimenter.ideal_observer = m.io.VOT_f0.AA, 
      test.n_block = n.test_block, 
      n_test = n.test.token, 
      category_dist_ratio1 = dist.L2_category.cue1, 
      category_dist_ratio2 = dist.L2_category.cue2, 
  ...
  p.AA.bias <- make_results_plot_decision_making(data = bias.pred, data_best = bias.res$best)
  ggsave(paste0('../figures/plotly/AA_cases/bias_', example_label, '.png'), plot = p.AA.bias, width = 2*(base.width*length(levels(factor(bias.pred$beta_pi))) + .5), height = 2*(base.height*length(levels(factor(bias.pred$lapse_rate))) + .5), dpi = 300)
...
select_three_best_models <- function(d, example_label = "Cue_reweighting"){
  ...
  d.best <- bind_rows(
    ...
    d %>% 
      ...
      filter(category == Item.Intended_category & lapse_rate %in% lapse_rate[which.min(abs(lapse_rate - data_best2$par[1]))] & 
...
d.AA.results.contrast_shift  <- construct_three_models(example_label = "Contrast_shift", experimenter.ideal_observer = m.io.VOT_f0.AA)
...
d.AA.results.contrast_reduction  <- construct_three_models(example_label = "Contrast_reduction", experimenter.ideal_observer = m.io.VOT_f0.AA)
...
d.AA.results.contrast_collapse  <- construct_three_models(example_label = "Contrast_collapse", experimenter.ideal_observer = m.io.VOT_f0.AA)
...
d.AA.results.best.cases <- bind_rows(
  ...
  mutate(parameterization = case_when(
   ...
   ModelType == "decision-making" ~ paste0("lambda = ", lapse_rate, ", beta = ", beta_pi),
