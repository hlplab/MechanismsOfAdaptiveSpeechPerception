...
```{r PR-result-changes-in-representations-ASA, fig.width= base.width, fig.height= base.height +.5, fig.cap="(ref:PR-result-changes-in-representations)"}
p.PR.results <-
  d.PR %>%
  filter(between(as.numeric(as.character(prior_kappa)), 75, 780) & between(as.numeric(as.character(prior_nu)), 160, 1000)) %>%
  filter(category == categories.PR[1]) %>%
  ggplot(aes(x = observationID, y = response, color = Condition, l = Condition)) +
  geom_line(aes(group = Condition), alpha = .75) +
  # , breaks = 1:length(unique(d.PR$observationID)), labels = c(categories.PR[1], rep("", length(unique(d.PR$observationID)) - 2), categories.PR[2])
...
```{r PR-result-changes-in-decision-making-ASA, fig.width= base.width, fig.height= base.height +.5, fig.cap="(ref:PR-result-changes-in-decision-making)"}
d.PR %<>% 
  filter(beta_pi == 0.01 & posterior.lapse_rate == .0005)
p.PR.results.bias <- p.PR.results %+% 
  d.PR +
  geom_text(
    data = . %>%
