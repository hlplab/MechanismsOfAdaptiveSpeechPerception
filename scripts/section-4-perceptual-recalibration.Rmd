...
```{r kraljic-samuel-2007-replotted, fig.width = phonetic.panel.width + 1.7, fig.height = phonetic.panel.height + 0.6, fig.cap="(ref:kraljic-samuel-2007-replotted)"}
...
p20 <- replot_perceptual_recalibration_result(
  ...
  ggh4x::force_panelsizes(cols = phonetic.panel.width, rows = phonetic.panel.height) + theme(legend.position = "right")

p20

ggsave(paste0('../figures/plotly/kraljic-samuel-2007-replotted.png'), plot = p20, width = phonetic.panel.width + 1.7, height = phonetic.panel.height + .58, dpi = 300)
```
...
## Data
Most published experiments on perceptual recalibration do *not* measure and report the acoustic properties of their stimuli. We therefore simulate an example experiment on perceptual recalibration to shifted /d/ and /t/, following the same stimulus creation and selection procedure employed in experiments on perceptual recalibration [e.g., @kraljic-samuel2006].^[A reviewer pointed out that perceptual recalibration to shifted /d/ or /t/ does not appear to be talker-specific (for L1-US English listeners): listeners that are exposed to one talker with shifted /d/ or /t/ generalize perceptual recalibration to test tokens from a different talker [unlike for shifted /s/ or /`r linguisticsdown::cond_cmpl("ʃ")`/, e.g., @kraljic-samuel2007]. We consider questions about talker-specificity to be orthogonal to the questions discussed here: any of the three mechanisms integrated in ASP can be talker-specific or talker-independent, or talker-specificity can depend on the type of phonetic contrast or the type of cue (for further discussion, see SI \@ref(sec:cautionary-note)).] The details of our stimulus generation approach are described in the SI (\@ref(sec:SI-PR)), and do not affect the general results presented below. They are merely meant to provide a sufficiently concrete example. Readers can revisit any of the assumptions we made by downloading the [R markdown document that this article is generated from](https://osf.io/q7gjp/). 
...
get_locations_along_phonetic_contrast <- function(ideal_observer, target_probabilities = .5) {
  ...
  for (t in target_probabilities) {
   # For each location, get distance from the desired shift for that category
    probability_of_category1_in_its_lexical_context_at_location <- abs(t - get_experimenter_posterior_of_category1(x = locations, model = ideal_observer))
...
make_perceptual_recalibration_exposure_design = function(
  ...
  location.shifted_category1 <- 
    get_locations_along_phonetic_contrast(
     ideal_observer = experimenter.ideal_observer %>% mutate(prior = c(exposure.category_prior_of_lexical_context, 1 - exposure.category_prior_of_lexical_context)),
     target_probabilities = exposure.token_shift)
  
  location.shifted_category2 <- get_locations_along_phonetic_contrast(
    ideal_observer = experimenter.ideal_observer %>% mutate(prior = c(1 - exposure.category_prior_of_lexical_context, exposure.category_prior_of_lexical_context)),
    target_probabilities = exposure.token_shift)
...
```{r PR-exposure-test-plot, fig.width = phonetic.panel.width*3+1, fig.height = phonetic.panel.height + 1, fig.cap="(ref:PR-exposure-test-plot)"}
p.PR.exposure <- 
  d.PR.exposure %>%
  ggplot(aes(x = VOT, y = f0_Mel, color = Item.Category)) +
  geom_point(aes(shape = Item.Type), alpha = .5) +
  geom_segment(
    data = . %>%
      mutate(
        Condition = case_when(
  ...
  myGplot.defaults(base_size = phonetic.base.size, set_theme = F) +
  ggh4x::force_panelsizes(cols = phonetic.panel.width, rows = phonetic.panel.height) + theme(legend.position = "top")
...
prow = plot_grid(
    p.PR.exposure + theme(legend.position="none"),
    p.PR.test + theme(legend.position="none"),
...
ggsave(paste0('../figures/plotly/PR-exposure-test-plot.png'), plot = p1, width = phonetic.panel.width*3 + 1.2, height = phonetic.panel.height + 1, dpi = 300)
```
...
```{r PR-result-changes-in-representations, fig.width = result.panel.size*5 + 1, fig.height = result.panel.size* 5 + 1.5, fig.cap="(ref:PR-result-changes-in-representations)"}
p.PR.results <-
  d.PR %>%
  filter(category == categories.PR[1]) %>%
...
ggsave(paste0('../figures/plotly/PR-result-changes-in-representations.png'), plot = p1, width = result.panel.size*5 + 1, height = result.panel.size* 5 + 1.5, dpi = 300)
```
...
```{r PR-result-changes-in-representations-categories, fig.width = phonetic.panel.width*4 + 1.5, fig.height = phonetic.panel.height*2 + 2, fig.cap="(ref:PR-result-changes-in-representations-categories)"}
levels <- c(.68, .95)

p1 <- plot_expected_categories_contour2D(
  x = d.PR %>% 
...
ggsave(paste0('../figures/plotly/PR-result-changes-in-representations-categories.png'), plot = p1, width = phonetic.panel.width*4 + 1.5, height = phonetic.panel.height*2 + 1.8, dpi = 300)
```
...
```{r PR-result-changes-in-decision-making, fig.width = result.panel.size*5 + 1, fig.height = result.panel.size* 5 + 1.5, fig.cap="(ref:PR-result-changes-in-decision-making)"}
p.PR.results.bias <- 
  p.PR.results %+% 
  d.PR +
  geom_text(
...
ggsave(paste0('../figures/plotly/PR-result-changes-in-decision-making.png'), plot = p.PR.results.bias, width = result.panel.size*5 + 1, height = result.panel.size* 5 + 1.5, dpi = 300)
```
...
(ref:PR-test-normalization) Effects of changes in normalization on the perception of test locations, depending on the relative weighting of previous experience ($\kappa_0$) during the inference of the cue mean during exposure. Normalization based on exposure results in a shift in the perception of the test stimuli. The magnitude of that shift depends on $\kappa_0$, with larger shifts (more learning) for smaller $\kappa_0$ (see Equation \@ref(eq:normalization-change)). Color indicates the posterior probability of /d/ for each test token predicted by the categorization model after normalization (from red = 0 to blue = 1).

```{r PR-test-normalization, fig.width= base.width * 2, fig.height= base.height * (if (ANIMATE_FIGURES) 1 else 2) + (if (ANIMATE_FIGURES) 1 else 0), fig.cap="(ref:PR-test-normalization)", fig.show=if (ANIMATE_FIGURES) 'animate' else 'asis'}
d.PR <- 
  d.PR.exposure %>%
...
if (ANIMATE_FIGURES) {
  ...
  # ggsave(paste0('../figures/plotly/PR-test-normalization.png'), plot = p1, width = phonetic.panel.width*2 + 1.6, height = phonetic.panel.height*3 + 2.2, dpi = 300)
  ggsave(paste0('../figures/plotly/PR-test-normalization.png'), plot = p1, width = phonetic.panel.width*2 + 0.85, height = phonetic.panel.height*3 + 1, dpi = 300)
}
...
if (SAVE_ANIMATED_VIDEO) {
  ...
  anim_save(
    "demonstrate-PR-test-normalization.webm", 
    animation = animate(
      p.animation + 
        geom_text(aes(color = Prior_proportion_category1), size = 10) + 
        theme(
...
```{r PR-result-changes-in-normalization, fig.width= result.panel.size*5 + 0.8, fig.height = result.panel.size + 1.2, fig.cap="(ref:PR-result-changes-in-normalization)"}
...
ggsave(paste0('../figures/plotly/PR-result-changes-in-normalization.png'), plot = p.PR.results.normalization, width = result.panel.size*5 + 0.8, height = result.panel.size + 1.2, dpi = 300)
```
