...
get_average_bias_results_across_simulations <- function(data){
  # output <-    # this was the original data that get logged into history.optimization.bias. It computes the mean & se of the response for each token across simulations; and then in the logging stage, an average of the mean across tokens was computed as the mean accuracy for a parameterization
  #   data %>%
  ...
  output <- 
    ...
    relocate(
     !!! syms(setdiff(names(.), c("response.mean.mean","response.mean.se", "response.n_sims"))),
     response.mean.mean, response.mean.se, response.n_sims) %>%
    ungroup()
...
## Summary
Paralleling Case Study 1 on perceptual recalibration, we find that any of the three change mechanisms can qualitatively explain the signature results of experiments on accent adaptation---at least for the hypothetical L2 accent considered in Case Study 2. Despite the fact that this L2 accent differs from L1 listeners’ prior expectations in complex ways---specifically, in terms of how it uses both VOT and f0---even computationally simple, non-representational change mechanisms (normalization and decision-making) can explain qualitative improvements in the perception of this L2 accents. For example, contrary to common assumptions, changes in decision-making can lead to _overall_ improvements in recognition accuracy, not just zero-sum trade-offs. This supports the conclusion of Case Study 1: at the level of analysis that is commonly applied in previous work (and thus in our case studies), experiments on accent adaptation do not necessarily rule out any of the three mechanisms.

For the hypothetical L2 accent considered in Case Study 2, the three change models do not only make similar *qualitative* predictions (e.g., improved performance after L2-accented exposure), they can also yield highly comparable *quantitative* performance: recognition accuracies of the best-performing models in Figures \@ref(fig:AA-result-changes-in-representations)-\@ref(fig:AA-result-changes-in-normalization) range from .91 for changes in decision-making to .93 for changes in category representations. Intuitively, quantitatively similar maximum performance makes it particularly difficult to distinguish between the three change models, or combinations thereof.<!-- ^[In this example, we are referring to the best-performing parameterizations of each change model. We emphasize again that this is not necessarily the same as the parameterization that best fits human behavior. Determining those best-*fitting* parameterization is the same as determining the relative engagement of the three mechanisms, and thus is the endeavor for future research that the present article aims to support through the development of ASP.]-->
...
<!-- STHHERE IS WRONG. WE NEED TO SAY THE OBIOUS: IMPROVEMENTS AFTER EXPOSURE TO SOME TYPES OF ACCENTS ACTUALLY *DO* RULE OUT SOME MECHANISMS. THUS, IF RESEARCHERS BOTHERED TO DERIVE PREDICTIONS BASED ON THE SPECIFIC ACCENT SCENARIO/PHONETIC PROPERTIES, IT WOULD BE POSSIBLE TO RULE OUT SOME MECHANISMS. AND INTUITIVELY, THIS MIGHT ALSO HELP SHED LIGHT ON WHAT COMBINATIONS OF MECHANISMS ARE AT WORK.

ALSO REFRAME POINTS:
"ACCENT SCENARIO" -- "L1-L2 DIFFERENCES?

1) DIFFICULTY PRIOR
2) HOW MUCH A MECHANISMS CAN HELP DIFFERS BASED ON L1-L2 DIFFERENCES. NULL EFFECT IS EXTREME EXAMPLE OF THAT
3) CRITICALLY, WHETHER AND HOW MUCH A MECHANISM PREDICTS IMPROVEMENTS DEPENDS ON L1-L2 DIFFERENCES (INCL. WHICH MECHANISM 'WINS')
...
Second, the same change model can support more or less overall accuracy improvements (as gleaned from comparing the panels within each column), depending on the specific differences between the L2 and L1 accents. Consider the case of contrast shift (middle row). While changes in representations and normalization both predict large improvements in overall recognition accuracy in the L2-accented exposure, changes in decision-making do not (i.e., the difference in the predicted accuracy across L1- and L2-accented exposure conditions is negligible).<!-- ^[Note that the result predicted for changes in decision-making when applied to the contrast shift scenario is an example of the type of zero-sum trade-offs often attributed to this change mechanism (i.e., the recognition of /d/ improves at the cost of the recognition of /t/). The heterogeneity of the results across the three scenarios makes it clear that the trade-off is not an inherent property of changes in decision-making. It is instead a possible outcome arising from the specific way in which the L1 and L2 accent differ.]-->
