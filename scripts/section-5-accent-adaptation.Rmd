...
find_best_model.bias.AA <- function() {
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
    ...
    if (best_performing_parameters.bias.gridSearch$lapse_rate == 0){
      # if the best-performing lapse rate is 0, then only optimize over beta pi
     ...
     best_performing_parameters.bias.optimization$par <- c(0, exp(best_performing_parameters.bias.optimization$par[1]))
       ...
       best_performing_parameters.bias.optimization <-
        optim( 
          # if the best-performing lapse rate is not 0, then optimize over both lapse rate and beta pi
    ...
    history.bias.optimization <- history.optimization_bias
    
    saveRDS(best_performing_parameters.bias, get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
    saveRDS(history.bias.optimization, get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds")))
  } else {
    best_performing_parameters.bias <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
    history.bias.optimization <- readRDS(get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds")))
  }
