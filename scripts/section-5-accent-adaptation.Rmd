...
temp <- as.matrix(pivot_wider(history.optimization_bias %>% select(beta_pi, lapse_rate, accuracy), names_from = beta_pi, values_from = accuracy)[,-1])

x <- seq(min(range.lapse_rate), max(range.lapse_rate), length = 10) 
y <- exp(seq(min(log(range.beta_pi)), max(log(range.beta_pi)), length = 10))

history.optimization_bias.list <- list(lapse_rate = x, beta_pi = y, accuracy = temp)
  plot_ly() %>%
    add_surface(
      y = history.optimization_bias.list$lapse_rate, x = history.optimization_bias.list$beta_pi, z = temp,
      opacity = 0.2,
    ...
    layout(
      scene = list(
        ...
        camera = list(
          ...
          up =  list(x = 0, y = 0, z = 1)),
        zaxis = list(title = "Overall accuracy", titlefont = list(size = 22)), 
