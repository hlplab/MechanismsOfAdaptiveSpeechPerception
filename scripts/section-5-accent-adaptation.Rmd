...
```{r DEBUG-AA-models-changes-in-decision-making-grid-search, echo=FALSE, message=FALSE, warning=FALSE, eval= FALSE}
library(NMOF) #DEBUG
start_time <- Sys.time() #DEBUG
res2 <- gridSearch(get_accuracy_from_updated_bias, #DEBUG,
                  npar = 2,   ## length of x
                  lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
                  upper = c(max(range.lapse_rate), max(log(range.beta_pi))),
                  n = 10)      ## number of levels per element in x
## 2 variables with 5, 5 levels: 25 function evaluations required.
end_time <- Sys.time() #DEBUG
end_time - start_time #DEBUG

res2 <- readRDS(file = "../res2")
saveRDS(history.optimization_bias, "../models/history.optimization_bias_grid_search_min=50_max=100.rds")
```


```{r DEBUG-AA-models-changes-in-decision-making-grid-search-visualization, echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
...
construct_accent_models2 <- function(
    example_label = "Cue_reweighting",
    experimenter.ideal_observer,
    category_dist_ratio1 = dist.L2_category.cue1,
    category_dist_ratio2 = dist.L2_category.cue2,
...
start_time <- Sys.time() #DEBUG
temp.gridSearch <- construct_accent_models2(example_label = example_label, experimenter.ideal_observer = m.io.VOT_f0.AA)
  end_time <- Sys.time() #DEBUG
  end_time - start_time #DEBUG
