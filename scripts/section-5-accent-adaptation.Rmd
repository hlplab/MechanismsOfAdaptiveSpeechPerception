  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
    ...
    best_performing_parameters.bias <-
      optim( # DEBUG
        ...
        par = c(log(2.2)),
        fn = get_accuracy_from_updated_bias_beta_only, #DEBUG
        method = "SANN", 
      #  method = "L-BFGS-B", # DEBUG
        # lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
        # upper = c(max(range.lapse_rate), max(log(range.beta_pi))),
        lower = c(min(log(range.beta_pi))), # DEBUG
        upper = c(max(log(range.beta_pi))), # DEBUG
      control = list(fnscale = -1, trace = 1, temp = 100, tmax = 100, maxit = 20000)
        # control = list(
        #   fnscale = -1,
        # #  factr = 10^8))
        #   #DEBUG
...
# history.optimization_bias.temp <- readRDS(get_path(paste0("../models/d.AA.history.optimization.bias_Cue_reweighting_min=50_max=100_L-BFGS-B.rds"))) # DEBUG
# history.optimization_bias.temp2 <- readRDS(get_path(paste0("../models/d.AA.history.optimization.bias_Cue_reweighting_min=50_max=100_SANN.rds")))# DEBUG
# history.optimization_bias.temp %<>% mutate(beta_pi2 = sprintf("%.6f", beta_pi), accuracy2 = sprintf("%.6f", accuracy))
# history.optimization_bias.temp2 %<>% mutate(beta_pi2 = sprintf("%.10f", beta_pi), accuracy2 = sprintf("%.10f", accuracy))
# 
# temp <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_Cue_reweighting_min=50_max=100_SANN.rds")))
# sprintf("%.6f", temp$value)
