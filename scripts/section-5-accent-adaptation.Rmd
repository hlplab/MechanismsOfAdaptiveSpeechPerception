...
```{r AA-result-changes-in-representations-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE, eval = FALSE}
data_best = best_performing_parameters.representations

make_results_plot_representations(data = d.AA.representations %>% 
                                    filter(prior_kappa %in% prior_kappa[which.min(abs(as.numeric(as.character(prior_kappa)) - data_best$par[1]))] & #highlight the parameters (within the plotted range) that are closest to the best performing parameterization
                  prior_nu %in% prior_nu[which.min(abs(as.numeric(as.character(prior_nu)) - data_best$par[2]))]), data_best = best_performing_parameters.representations) + 
  scale_fill_manual(breaks = c(T, F), values = c("white", "white"))
```
...
```{r save-best-performing-models-changes-in-decision-making-from-grid-search-place-holder, echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
best_performing_parameters.bias = list()
best_performing_parameters.bias$par <- c(res_bias.gridSearch$minlevels[1], exp(res_bias.gridSearch$minlevels[2]))
...
find_best_model.bias.AA <- function() {
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
    ...
    saveRDS(best_performing_parameters.bias.optimization, get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
    saveRDS(history.optimization_bias, get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds")))
  } else {
    best_performing_parameters.bias.optimization <- readRDS(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))
    history.optimization_bias <- readRDS(get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds")))
  }
  
  return(best_performing_parameters.bias.optimization)
}
...
make_results_plot_decision_making(data = get_average_bias_results_across_simulations.byItem(d.AA.bias), data_best = best_performing_parameters.bias)
```


```{r AA-result-changes-in-decision-making-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE, eval = FALSE}
data_best = best_performing_parameters.bias
d.AA.bias.highlight <- d.AA.bias %>% 
  filter(posterior.lapse_rate %in% posterior.lapse_rate[which.min(abs(posterior.lapse_rate - data_best$par[1]))] & 
                  beta_pi %in% beta_pi[which.min(abs(beta_pi - data_best$par[2]))])

make_results_plot_decision_making(data = get_average_bias_results_across_simulations.byItem(d.AA.bias.highlight), data_best = best_performing_parameters.bias) + 
  scale_fill_manual(breaks = c(T, F), values = c("white", "white"))
```
...
```{r AA-result-changes-in-normalization-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE, eval = FALSE}
make_results_plot_normalization(d.AA.normalization %>% 
  filter(prior_kappa.normalization %in% prior_kappa.normalization[which.min(abs(as.numeric(as.character(prior_kappa.normalization)) - best_performing_parameters.normalization$par[1]))]), best_performing_parameters.normalization) + 
  scale_fill_manual(breaks = c(T, F), values = c("white", "white"))
```
...
temp <- readRDS(file = "../models/best_performing_parameters.bias_Contrast_shift.rds")
```
...
temp <- readRDS(file = "../models/best_performing_parameters.bias_Contrast_reduction.rds")
```
...
temp <- readRDS(file = "../models/best_performing_parameters.bias_Contrast_collapse.rds")
temp2 <- readRDS(file = "../models/d.AA.history.optimization.bias_Contrast_reduction.rds")
```
...
```{r AA-plot-results-additional-cases, fig.width=base.width*4 + .5, fig.height=base.height*4 + .5, fig.cap = "(ref:AA-plot-results-additional-cases)", warning=FALSE}
...
```{r replot-sleep-LCN-data}
d.sleep <- read.csv(file = "/Users/xxie14/Dropbox/Projects/Completed/Sleep_studies_LCN/SleepAA/Results/Exp1/test_data.csv") %>% 
  filter(Speaker == 5) %>%
...
d.jephpp <- read.csv(file = "/Users/xxie14/Documents/github_projects/computational_modeling_of_accent_adaptation_swedish_english-master/data/English/English_perception_identification_Xie et al 2017.csv") %>% 
  mutate(Condition = case_when(
    Group == "control" ~ "L1-accented",
    Group == "experimental" ~ "L2-accented"
  ),
  Item.Intended_category = case_when(
    Sound == "d" ~ "/d/",
    Sound == "t" ~ "/t/"
  ),
  response = Spelling1.ACC,
  category = Item.Intended_category) 

basic_AA_result_plot(d.jephpp)+
  geom_text(
      inherit.aes = FALSE, 
      data = . %>%
        group_by(Condition) %>%
