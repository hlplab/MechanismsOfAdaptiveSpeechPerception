...
```{r AA-result-changes-in-representations-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE}
...
get_accuracy_from_updated_bias <- function( #this function is used when both parameters (lapse_rate, beta_pi) are considered during grid search
  ...
  return(-accuracy) # here the opposite of accuracy is taken because grid search implements minimization and we would like to maximize accuracy
...
find_best_model.bias.AA <- function() {
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
    ...
    best_performing_parameters.bias.optimization <-
      optim( 
        ### using the commented out lines if one wishes to optimize over both lapse rate and beta pi
   ...
   best_performing_parameters.bias.optimization$par <- c(0, exp(best_performing_parameters.bias.optimization$par[1]))
    best_performing_parameters.bias.optimization$value %<>% exp(.)
...
```{r AA-result-changes-in-decision-making-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE}
...
```{r AA-result-changes-in-normalization-plot-for-ASA, fig.width= base.width, fig.height= base.height + 1, fig.cap="(ref:AA-result-changes-in-representations-ASA)", warning=FALSE}
