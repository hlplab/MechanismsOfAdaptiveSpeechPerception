...
```{r save-best-performing-models-changes-in-decision-making-from-grid-search, echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}

# TO-DO: temporarily use best-performing parameters from grid search for all accent scenarios
#these files exist for the three additional cases
# history.optimization_bias.gridSearch <-  readRDS(get_path(paste0("../models/history.optimization_bias_gridSearch_", example_label,".rds")))
# res_bias.gridSearch <- readRDS(get_path(paste0("../models/res.optimization_bias_gridSearch_", example_label,".rds")))
...
find_best_model.bias.AA <- function() {
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))) {
   ...
   best_performing_parameters.bias.optimization$par <- c(0, exp(best_performing_parameters.bias.optimization$par[1]))
    ...
    print(paste0("history has rows", nrow(history.bias.optimization))) # DEBUG
...
```{r compare-AA-models-best-three-functions-DEBUG, eval = FALSE}
simulation.control <-
  list(
    min.simulations = 50, 
    max.simulations = 100, 
...
grid_search_for_bias_model <- function(
  ...
  res <- NMOF::gridSearch(
    get_accuracy_from_updated_bias, #DEBUG,
    npar = 2,   ## length of x
    # method = "multicore",
    # mc.control = list(mc.cores = detectCores()),
    lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
  ...
  saveRDS(res, paste0("../models/res.optimization_bias_gridSearch_", example_label, '.rds'))
    ...
    history.optimization_bias <- readRDS(paste0("../models/history.optimization_bias_gridSearch_", example_label, '.rds'))
    res <- readRDS(paste0("../models/res.optimization_bias_gridSearch_", example_label, '.rds'))
  }
...
temp.gridSearch <- grid_search_for_bias_model(example_label = example_label, experimenter.ideal_observer = m.io.VOT_f0.AA)
  end_time <- Sys.time() #DEBUG
  end_time - start_time #DEBUG

  
# visualize_bias_parameter_space(data = history.optimization_bias.77,
#                                range.lapse_rate = range.lapse_rate,
#     range.beta_pi = range.beta_pi) %>%
#   layout(
#       scene = list(
...
#           up =  list(x = 0, y = 0, z = 1)),
#         zaxis = list(range = c(0.4,0.6))))
# 
# example_label = "Contrast_shift"
# history.optimization_bias.77 <- readRDS(paste0("../models/history.optimization_bias_gridSearch_", example_label, '77.rds'))
# res.77 <- readRDS(paste0("../models/res.optimization_bias_gridSearch_", example_label, '77.rds'))
# 
# example_label = "Contrast_reduction"
# history.optimization_bias.contrast_reduction <- readRDS(paste0("../models/history.optimization_bias_gridSearch_", example_label, '.rds'))
# res.contrast_reduction <- readRDS(paste0("../models/res.optimization_bias_gridSearch_", example_label, '.rds'))
```
...
construct_accent_models <- function(
  ...
  history.optimization_bias <<- tibble(.rows = 0)
  best_performing_parameters.bias <- find_best_model.bias.AA() #TO-DO: doubleck check the logged beta to see if it is in raw space or log space
