      ...
      layout(
        scene = list(
          ...
          camera = list(
            ...
            up =  list(x = 0, y = 0, z = 1)),
          zaxis = list(title = "Overall accuracy", titlefont = list(size = 22), range = c(0,1), tickvals = c(0, 0.2 ,0.4, 0.6, 0.8, 1)), 
          yaxis = list(title = "lapse_rate",titlefont = list(size = 22)),
          xaxis = list(title = "beta_pi", titlefont = list(size = 22))))
}
```
...
p.bias.gridSearch <- visualize_bias_parameter_space(data = d.bias.gridSearch,
                                range.lapse_rate = range.lapse_rate, 
                                range.beta_pi = range.beta_pi) 
best_parameter.bias.gridSearch <- d.bias.gridSearch %>%
...
find_best_model.bias.AA <- function() {
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/d.AA.history.optimization.bias_", example_label,".rds")))) {
    ...
    best_performing_parameters.bias <-
      optim( 
        ...
        # lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
        # upper = c(max(range.lapse_rate), max(log(range.beta_pi))),
        par = c(log(best_parameter.bias.gridSearch$beta_pi)),
    ...
    best_performing_parameters.bias$par <- c(0, exp(best_performing_parameters.bias$par[1]))
    best_performing_parameters.bias$value %<>% exp(.)
