...
get_accuracy_from_updated_bias_beta_only <- function(#this function is used when lapse_rate is set to zero and only beta_pi is varied during optimization search
    pars,
    model = m.io.VOT_f0.AA,
    exposure = d.AA.exposure %>% filter(Condition == "L2-accented", Subject == paste0(Condition, ".1")), # find the best-performing parameterization for the L2-accented condition
    test = d.AA.test %>% filter(Condition == "L2-accented", Subject == paste0(Condition, ".1")),
...
grid_search_for_bias_model <- function(
  ...
  res <- NMOF::gridSearch(get_accuracy_from_updated_bias,
                    npar = 2,   ## length of x
                    # method = "multicore", # if one is not interested in saving se of accuracy and log them in a history file, then multicore can be used to speed up the grid search; "method" and "mc.contro" need to be specified
...
visualize_bias_parameter_space <- function(
  ...
  d.accuracy <- as.matrix(pivot_wider(data %>% select(beta_pi, lapse_rate, accuracy), names_from = beta_pi, values_from = accuracy)[,-1])
    ...
    plot_ly() %>%
      ...
      layout(
        ...
        scene = list(
          ...
          aspectratio = list(x = 1.8, y = 1.9, z = 1.6), 
          # perspective good for showing categorization curve
          camera = list(
            eye = list(x = -1.25, y = -3.25, z = 0.5), 
            up =  list(x = 0, y = 0, z = -1)),
          zaxis = list(title = "Overall<br>accuracy", titlefont = list(size = 16), range = c(0,1), tickvals = c(0, 0.2 ,0.4, 0.6, 0.8, 1)), 
...
grid_search_for_bias_model <- function(
  ...
  res <- NMOF::gridSearch(get_accuracy_from_updated_bias, #DEBUG,
                    npar = 2,   ## length of x
                    # method = "multicore",
                    # mc.control = list(mc.cores = detectCores()),
                    lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
...
construct_accent_models <- function(
  ...
  if (select_best_only == T){
    representations.pred %<>%
      filter(prior_kappa %in% prior_kappa[which.min(abs(as.numeric(as.character(prior_kappa)) - best_performing_parameters.representations$par[1]))] & #highlight the parameters (within the plotted range) that are closest to the best performing parameterization
  ...
  bias.pred %<>% get_average_bias_results_across_simulations.byItem() # average predictions across simulations for the bias model
...
var.ratio <- 1
example_label = "Contrast_reduction"
d.AA.results.contrast_reduction.best  <- construct_accent_models(example_label = "Contrast_reduction", experimenter.ideal_observer = m.io.VOT_f0.AA)
```
...
d.AA.results.contrast_collapse.best  <- construct_accent_models(example_label = "Contrast_collapse", experimenter.ideal_observer = m.io.VOT_f0.AA)
```
...
shift.cue2 <- 0.22
var.ratio <- 1
d.AA.results.cue_reweighting.best  <- construct_accent_models(example_label = "Cue_reweighting", experimenter.ideal_observer = m.io.VOT_f0.AA)
```
