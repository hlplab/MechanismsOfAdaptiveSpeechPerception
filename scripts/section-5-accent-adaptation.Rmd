...
get_parameters_phonetic_contrast.AA <- function(
    ideal.observer,
   ...
   # The larger the category_dist_ratio, the greater distance along that cue dimension (1 = VOT, 2 = f0) 
   # between the two categories in the L2 accent. If category_dist_ratio1 = category_dist_ratio2 = 0, 
   # then the two categories overlap entirely in the L2 accent. If category_dist_ratio1 = category_dist_ratio2 = 1, 
   # then /d/-/t/ will have the same distance as that in L1-accent along the primary cue dimension.
...
make_exposure_test_plot <- function(
  ...
  prow = plot_grid(
    ...
    labels = c('A)', 'B)'), nrow = 1, rel_widths = c(2.1,1.25))
...
```{r AA-exposure-test-plot, fig.width=phonetic.panel.width*3+1, fig.height = phonetic.panel.height + 1, fig.cap="(ref:AA-exposure-test-plot)", warning=FALSE}
p1 <- make_exposure_test_plot(exposure.data = d.AA.exposure, test.data = d.AA.test)
p1
ggsave(paste0('../figures/plotly/AA-exposure-test-plot.png'), plot = p1, width = phonetic.panel.width*3 + 1.2, height = phonetic.panel.height + 1, dpi = 300)
```
...
basic_AA_result_plot <- function(data) {
  data %>%
    ...
    list(
      ...
      myGplot.defaults(base_size = result.base.size, set_theme = F),
      theme(
...
(ref:AA-result-changes-in-representations) Predictions of a learning model that derives accent adaptation as changes in category representations. Predicted categorization accuracies for the L2-accented test tokens after `r conditions.AA[1]` or `r conditions.AA[2]` exposure, as a function of the strength of the prior beliefs in category means ($\kappa_{c,0}$) and covariances ($\nu_{c,0}$). The average accuracy across /d/ and /t/ is shown above the bars for each exposure condition. Error bars show 95% bootstrapped confidence intervals. The highlighted panel is the one closest to the best-performing parameterization ($\kappa_{c,0} = `r best_performing_parameters.representations$par[1]`$; $\nu_{c,0}=`r best_performing_parameters.representations$par[2]`$; overall accuracy $=`r best_performing_parameters.representations$value`$). 
...
make_results_plot_representations <- function(data, data_best) {
  p.results <- 
    ...
    geom_text(
      ...
      size = convert_fontsize_to_textsize(result.base.size)) +
    facet_grid(
...
```{r AA-result-changes-in-representations, fig.width = result.panel.size*5 + 1, fig.height = result.panel.size*5 + 2, fig.cap="(ref:AA-result-changes-in-representations)", warning=FALSE}
p1 <- make_results_plot_representations(data = d.AA.representations %>% filter(prior_kappa %in% prior_kappa.plot & prior_nu %in% prior_nu.plot), data_best = best_performing_parameters.representations)
p1
ggsave(paste0('../figures/plotly/AA-result-changes-in-representations.png'), plot = p1, width = result.panel.size*5 + 1, height = result.panel.size* 5 + 1.5, dpi = 300)
```
...
get_accuracy_from_updated_bias <- function( #this function is used when both parameters (lapse_rate, beta_pi) are considered during grid search
    pars,
    model = m.io.VOT_f0.AA,
  ...
  history.optimization_bias <<-
    bind_rows(
      ...
      tibble(lapse_rate = lapse_rate, beta_pi = beta_pi, 
             ...
             accuracy = u.across_simulations$response, 
             accuracy.se = u.across_simulations$response.mean.se))
  
  accuracy = log(u.across_simulations$response)
  return(-accuracy) # here the opposite of accuracy is taken because grid search and the optim search implement minimization and we would like to maximize
 
}

# This function is used when lapse_rate is set to zero and only beta_pi is varied during optimization search
get_accuracy_from_updated_bias_beta_only <- function(
    pars,
    model = m.io.VOT_f0.AA,
    # find the best-performing parameterization for the L2-accented condition
    exposure = d.AA.exposure %>% filter(Condition == "L2-accented", Subject == paste0(Condition, ".1")), 
    test = d.AA.test %>% filter(Condition == "L2-accented", Subject == paste0(Condition, ".1")),
    control = simulation.control,
    verbose = FALSE
  ...
  return(-accuracy) # here the opposite of accuracy is taken because grid search and the optim search implement minimization and we would like to maximize accuracy
}
```

```{r grid-search-AA-models-changes-in-decision-making-functions, echo=FALSE, message=FALSE, warning=FALSE}
grid_search_for_bias_model <- function(
    example_label = "Cue_reweighting",
  ...
  if (RESET_MODELS || !file.exists(get_path(paste0("../models/history.optimization_bias_gridSearch_", example_label,".rds")))) {
    res <- NMOF::gridSearch(
      get_accuracy_from_updated_bias,
      npar = 2,   ## length of x
      # method = "multicore", # if one is not interested in saving se of accuracy and log them in a history file, 
      # then multicore can be used to speed up the grid search; "method" and "mc.control" need to be specified
      # mc.control = list(mc.cores = detectCores()),
      lower = c(min(range.lapse_rate), min(log(range.beta_pi))),
      upper = c(max(range.lapse_rate), max(log(range.beta_pi))),
      n = 10)      ## number of levels per element in each variable during the grid search.
    
  saveRDS(history.optimization_bias, paste0("../models/history.optimization_bias_gridSearch_", example_label, '.rds'))
  saveRDS(res, paste0("../models/res.optimization_bias_gridSearch_", example_label, '.rds'))

  } else {
    history.optimization_bias <- readRDS(paste0("../models/history.optimization_bias_gridSearch_", example_label, '.rds'))
  }
  return(history.optimization_bias)
}

visualize_bias_parameter_space <- function(
    data = data,
    range.lapse_rate = range.lapse_rate,
    range.beta_pi = range.beta_pi
){
  d.accuracy <- 
    as.matrix(
      pivot_wider(
        data %>% 
          select(beta_pi, lapse_rate, accuracy), 
        names_from = beta_pi, 
        values_from = accuracy)[,-1])

  x <- seq(min(range.lapse_rate), max(range.lapse_rate), length = 10) 
  y <- exp(seq(min(log(range.beta_pi)), max(log(range.beta_pi)), length = 10))
  
  history.optimization_bias.list <- list(lapse_rate = x, beta_pi = y, accuracy = d.accuracy)
    plot_ly() %>%
      add_surface(
        y = history.optimization_bias.list$lapse_rate, x = history.optimization_bias.list$beta_pi, z = d.accuracy,
        opacity = 0.2,
      ...
      layout(
        ...
        scene = list(
          ...
          aspectratio = list(x = 1.8, y = 1.8, z = 1.5), 
          # perspective good for showing categorization curve
          camera = list(
            eye = list(x = -1.25, y = -3.25, z = 0.5), 
            up =  list(x = 0, y = 0, z = -1)),
          zaxis = list(
            title = "Overall<br>accuracy", 
            titlefont = list(size = 16), 
            range = c(0,1), 
            tickvals = c(0, 0.2 ,0.4, 0.6, 0.8, 1)), 
          yaxis = list(title = "lapse rate", titlefont = list(size = 16)),
          xaxis = list(title = "beta pi", titlefont = list(size = 16))))
}
```

```{r grid-search-AA-models-changes-in-decision-making, echo=FALSE, message=FALSE, warning=FALSE}
history.optimization_bias.gridSearch <- grid_search_for_bias_model(example_label = example_label)

best_performing_parameters.bias.gridSearch <- history.optimization_bias.gridSearch %>%
  filter(accuracy == max(accuracy))
```

```{r functions-AA-models-changes-in-decision-making-optimization, echo=FALSE, message=FALSE, warning=FALSE}
find_best_model.bias.AA <- function() {
  if(SET_SEED) set.seed(42007)
  history.optimization_bias <<- tibble(.rows = 0)
    
  if (best_performing_parameters.bias.gridSearch$lapse_rate == 0){
    # if the best-performing lapse rate from the grid search is 0, then only optimize over beta pi
    best_performing_parameters.bias.optimization <-
        optim(
          par = c(log(best_performing_parameters.bias.gridSearch$beta_pi)),
          fn = get_accuracy_from_updated_bias_beta_only, 
          lower = c(min(log(range.beta_pi))),
          upper = c(max(log(range.beta_pi))), 
          method = "L-BFGS-B", 
          control = list(
            fnscale = 1,
            factr = 10^5))
      
     best_performing_parameters.bias.optimization$par <- c(0, exp(best_performing_parameters.bias.optimization$par[1]))
    } else {
   best_performing_parameters.bias.optimization <-
    optim( 
      # if the best-performing lapse rate from the grid search is not 0, then optimize over both lapse rate and beta pi
      par = c(best_performing_parameters.bias.gridSearch$lapse_rate,log(best_performing_parameters.bias.gridSearch$beta_pi)),
     ...
     best_performing_parameters.bias.optimization$par <- c(best_performing_parameters.bias.optimization$par[1], exp(best_performing_parameters.bias.optimization$par[2]))
    }
  
    best_performing_parameters.bias.optimization$value <-  exp(-(best_performing_parameters.bias.optimization$value))
    
  output = list("parameters" = best_performing_parameters.bias.optimization, "history" = history.optimization_bias)
  return(output)
}
```

```{r study-AA-models-changes-in-decision-making-optimization, echo=FALSE, message=FALSE, warning=FALSE}
read_output_bias <- function(){
    if (RESET_MODELS || !file.exists(get_path(paste0("../models/best_performing_parameters.bias_", example_label,".rds")))){
    output.bias <- find_best_model.bias.AA()
    
    best_performing_parameters.bias <- output.bias$parameters
    history.optimization_bias <- output.bias$history
  ...
  output <- list("parameters" = best_performing_parameters.bias, "history" = history.optimization_bias)
  return(output)
}

best_performing_parameters.bias <- read_output_bias()$parameters
```


```{r function-AA-models-changes-in-decision-making, echo=FALSE, message=FALSE, warning=FALSE}
...
get_average_bias_results_across_simulations.byItem <- function(data){
    output <- 
    data %>%
  ...
  p.results <- 
    ...
    geom_text(
      inherit.aes = FALSE, data = . %>%
        ...
        summarise(mAcc = round(mean(response), digits = 2)), aes(label = mAcc, x = Condition, y = 1.05), size = convert_fontsize_to_textsize(result.base.size)) +
    facet_grid(
...
```{r AA-result-changes-in-decision-making, fig.width = result.panel.size*5 + 1, fig.height = result.panel.size*5 + 2, fig.cap="(ref:AA-result-changes-in-decision-making)", warning=FALSE}
p1 <- make_results_plot_decision_making(data = get_average_bias_results_across_simulations.byItem(d.AA.bias), data_best = best_performing_parameters.bias)
p1
ggsave(paste0('../figures/plotly/AA-result-changes-in-decision-making.png'), plot = p1, width = result.panel.size*5 + 1, height = result.panel.size* 5 + 1.5, dpi = 300)
```
...
prior_kappa.normalization.plot <- c(4, 16, 64, 256, 1024) # specify what parameters to plot
...
```{r AA-result-changes-in-normalization, fig.width= result.panel.size * 5 + 1, fig.height= result.panel.size + 1.2, fig.cap="(ref:AA-result-changes-in-normalization)", warning=FALSE}
p1 <- make_results_plot_normalization(d.AA.normalization %>% filter(prior_kappa.normalization %in% prior_kappa.normalization.plot), best_performing_parameters.normalization)
p1
ggsave(paste0('../figures/plotly/AA-result-changes-in-normalization.png'), plot = p1, width = result.panel.size*5 + 0.8, height = result.panel.size + 1.2, dpi = 300)
```
...
construct_accent_models <- function(
    example_label = "Cue_reweighting",
    experimenter.ideal_observer,
    category_dist_ratio1 = dist.L2_category.cue1,
    category_dist_ratio2 = dist.L2_category.cue2,
  ...
  representations.pred <- get_representation_models_for_plot(
    # here one can select to plot just the best-performing parameterization or to visualize a range of 
    # parameters by specifying the values assigned to prior_kappa and prior_nu
    prior_kappa = round(best_performing_parameters.representations$par[1], digits = 0), 
    prior_nu = round(best_performing_parameters.representations$par[2], digits = 0))
  
  if (select_best_only == T){
    representations.pred %<>%
      # highlight the parameters (within the plotted range) that are closest to the best performing parameterization
      filter(
        prior_kappa %in% prior_kappa[which.min(abs(as.numeric(as.character(prior_kappa)) - best_performing_parameters.representations$par[1]))] & 
          prior_nu %in% prior_nu[which.min(abs(as.numeric(as.character(prior_nu)) - best_performing_parameters.representations$par[2]))])
  }
...
#  history.optimization_bias <<- tibble(.rows = 0)
  ...
  # here one can select to plot just the best-performing parameterization or to visualize a range of 
  # parameters by specifying the values assigned to lapse_rate and beta_pi
  bias.pred <- get_decision_making_models_for_plot(
    lapse_rate = round(best_performing_parameters.bias$par[1], digits = 2), 
    beta_pi = round(best_performing_parameters.bias$par[2], digits = 2))
  
  # average predictions across simulations for the bias model
  bias.pred %<>% get_average_bias_results_across_simulations.byItem() 
  
  if (select_best_only == T){
    bias.pred %<>%
      filter(
        posterior.lapse_rate %in% posterior.lapse_rate[which.min(abs(posterior.lapse_rate - best_performing_parameters.bias$par[1]))] & 
  ...
  if (select_best_only == T){
    normalization.pred %<>%
      filter(prior_kappa.normalization %in% prior_kappa.normalization[which.min(abs(as.numeric(as.character(prior_kappa.normalization)) - best_performing_parameters.normalization$par[1]))])
  }
  ...
  ggsave(
    paste0('../figures/plotly/representations_', example_label, '.best.png'), 
  ...
  ggsave(
    paste0('../figures/plotly/bias_', example_label, '.best.png'), 
    plot = p.AA.bias, 
  ...
  p.AA.normalization <- 
    make_results_plot_normalization(
      ...
      best_performing_parameters.normalization)
  ggsave(
    paste0('../figures/plotly/normalization_', example_label, '.best.png'), 
...
d.AA.results.contrast_shift.best  <- construct_accent_models(example_label = "Contrast_shift", experimenter.ideal_observer = m.io.VOT_f0.AA)
```

```{r AA-result-comparison-best-models-contrast-reduction, warning=FALSE}
...
d.AA.results.contrast_reduction.best <- 
  construct_accent_models(example_label = "Contrast_reduction", experimenter.ideal_observer = m.io.VOT_f0.AA)
```

```{r AA-result-comparison-best-models-contrast-collapse, warning=FALSE}
...
d.AA.results.contrast_collapse.best <- 
  construct_accent_models(example_label = "Contrast_collapse", experimenter.ideal_observer = m.io.VOT_f0.AA)
```

```{r AA-result-comparison-best-models-cue-reweighting, warning=FALSE}
# Replot the main scenario: cue reweighting
min_VOT = 5 # Set the minimal values for randomly generated test token cue values to make sure it is physiologically possible
example_label = "Cue_reweighting"
dist.L2_category.cue1 <- 1
dist.L2_category.cue2 <- 0.22
...
shift.cue2 <- 0.22
var.ratio <- 1
d.AA.results.cue_reweighting.best <- 
  construct_accent_models(example_label = "Cue_reweighting", experimenter.ideal_observer = m.io.VOT_f0.AA)
```
...
# simulating a case of cue reweighting: like Korean-accented English stops, as presented in case study as the main case
min_VOT = 5
var.ratio = 1
example_label = "Cue_reweighting"
d.AA.exposure.target.cue.reweighting <- 
  make_accent_adaptation_exposure_design(
    experimenter.ideal_observer = m.io.VOT_f0.AA, 
    category_dist_ratio1 = 1, 
    category_dist_ratio2 = 0.22, 
    shift_mean_1 = 0.5, 
    shift_mean_2 = 0.22, 
...
d.AA.exposure.target.cases <- rbind(
  ...
  mutate(Case = factor(Case, levels = c("Cue reweighting", "Contrast reduction", "Contrast shift", "Contrast collapse")))

# plot ellipses for actual exposure
p.AA.exposure <- 
  d.AA.exposure.target.cases %>%
  mutate(Title = "Exposure\ndistributions") %>%
  filter(Condition == "L2-accented") %>%
  ...
  stat_ellipse(
    data = d.AA.exposure.target.cases %>%
      mutate(Title = "Exposure\ndistributions") %>%
      filter(Condition == "L1-accented") %>%
      mutate(Group = factor(paste0(Condition, Item.Category))), 
    aes(fill = Group), level = .95, geom = "polygon") +
  scale_x_continuous(expression("VOT(ms)"), expand = expansion(mult = .1, add = 0)) +
  ...
  facet_grid(Case ~ Title, switch = "y") + 
  myGplot.defaults(base_size = result.base.size, set_theme = F) + 
  theme(legend.position = "top") +
  ggh4x::force_panelsizes(cols = result.panel.size, rows = result.panel.size)
```

```{r AA-plot-results-additional-cases, fig.width=result.panel.size*4+1, fig.height = result.panel.size*4 + 1.5, fig.cap = "(ref:AA-plot-results-additional-cases)", warning=FALSE}
d.AA.results.best.cases <- 
  bind_rows(
  d.AA.results.cue_reweighting.best %>%
    mutate(Case = "Cue reweighting"),
  d.AA.results.contrast_reduction.best %>%
  ...
  mutate(Case = factor(
    Case, 
    levels = c("Cue reweighting","Contrast reduction", "Contrast shift", "Contrast collapse"))) %>%  
...
p.results.best.cases <- 
  d.AA.results.best.cases %>%
  mutate(
    ModelType = factor(paste0("Changes in\n", ModelType), levels = c("Changes in\nrepresentations", "Changes in\ndecision-making", "Changes in\nnormalization"))) %>%
  ...
  geom_text(
    inherit.aes = FALSE, data = . %>%
      ...
      summarise(mAcc = round(mean(response), digits = 2)),
    aes(label = mAcc, x = Condition, y = 1.05), 
    size = convert_fontsize_to_textsize(result.base.size)) +
  coord_cartesian(ylim = c(0, 1.1)) + 
...
prow <- 
  plot_grid(
    p.AA.exposure + theme(legend.position="none") + ggh4x::force_panelsizes(cols = result.panel.size, rows = result.panel.size), 
...
legend_prow <- get_legend(p.results.best.cases + 
                            theme(legend.key.height= duo.panel.key,
        legend.position = "top"))
p1 <- plot_grid(legend_prow, prow, ncol = 1, rel_heights = c(.1, 1))

p1

ggsave(paste0('../figures/plotly/AA-plot-results-additional-cases.png'), plot = p1, width = result.panel.size*4+1, height = result.panel.size*4 + 1.5, dpi = 300)
```
