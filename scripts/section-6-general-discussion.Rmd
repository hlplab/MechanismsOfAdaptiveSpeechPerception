...
compare_models.AA <- function(data1, data2, data3, data.exposure, data.test, VOT_range, f0_range, n_points) {
    ...
    bind_rows(
      ...
      data2 %>%
        filter(category == Item.Intended_category) %>%
        filter(lapse_rate == posterior.lapse_rate.selected & beta_pi == beta_pi.selected) %>%
        droplevels() %>%
        distinct(Condition, Subject, lapse_rate, beta_pi, posterior, sim) %>%
      ...
      data3 %>%
        ...
        droplevels() %>%
        distinct(Condition, Subject, prior_kappa.normalization, mu_inferred, prior, posterior) %>%
...
#change the parameters below depending on simulation outcomes
prior_kappa.selected = 1024
prior_nu.selected = 1024
posterior.lapse_rate.selected = min(d.AA.bias$lapse_rate)
