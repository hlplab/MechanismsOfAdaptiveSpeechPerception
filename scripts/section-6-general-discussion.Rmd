...
d.temp <- d.AA.bias %>% filter(lapse_rate == 5e-04 & beta_pi == .8) %>% filter(category == Item.Intended_category) %>% distinct(Condition, Subject, data, lapse_rate, beta_pi, prior, posterior, posterior.categorization)

d.temp.results <- d.AA.bias %>% 
  filter(lapse_rate == 5e-04 & beta_pi == .8) %>% 
  filter(category == Item.Intended_category) %>%
