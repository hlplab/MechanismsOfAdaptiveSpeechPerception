...
compare_models.AA <- function(data1, data2, data3, data.exposure, data.test, VOT_range, f0_range, n_points) {
    ...
    bind_rows(
      ...
      data2 %>%
        ...
        droplevels() %>%
       # distinct(Condition, Subject, posterior.lapse_rate, beta_pi, posterior, sim) %>% #DEBUG
        distinct(Condition, Subject, posterior.lapse_rate, beta_pi) %>%
...
prior_kappa.selected = 256 #DEBUG CHANGE THIS TO AUTOMATICALLY USE the parameters from the simulations
