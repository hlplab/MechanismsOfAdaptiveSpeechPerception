...
p.comparison <- compare_models.AA(d.AA.representations, d.AA.bias, d.AA.normalization, d.AA.exposure, d.AA.test, VOT_range, f0_range, n_points)
```

```{r show-model-categorization-3D-plots-similar-accuracy, fig.ncol = 1, fig.show='hold',fig.align='center', out.width= c(rep("99%",4)), out.height=c("15%",rep("20%", 3)),  fig.cap="(ref:show-model-categorization-3D-plots-similar-accuracy)", fig.subcap=c('Model predictions in accuracy','Predicted categorization functions after L1-accented exposure', 'Predicted categorization functions after L2-accented exposure', 'Differences in categorization functions between exposure conditions')}
# make_plot_compare_models.AA(compare_model_label = "similar-accuracy")
#fig.width=base.width*3 + .5, fig.height=base.height*4 + .5,
# out.width= c(rep("99%",4)), out.height=c("20%",rep("30%", 3)),
filename = c()
for (p in 1:length(model_names)) {
  filename[p] = file.path(get_path('../figures/plotly/'), paste0('p.3d.categorization.model_', model_names[p], '_L1-accented', '_', compare_model_label, '.png'))
  filename[p + length(model_names)] = file.path(get_path('../figures/plotly/'), paste0('p.3d.categorization.model_', model_names[p], '_L2-accented', '_', compare_model_label, '.png'))
  ...
  # 
  # p.temp <-plot_grid(p.comparison, p.L1, p.L2, p.diff, ncol = 1)
  # ggsave('../figures/plotly/p.3d.categorization_comparison.png', plot = p.temp, width = 2*(base.width*3 + .5), height = 4*(base.height + .5), dpi = 300)
  # 
#   plot_grid(
#   p.comparison, p1.L1, p2.L1, p3.L1, p1.L2, p2.L2, p3.L2, p1.diff, p2.diff, p3.diff, 
#   align = 'hv',
#   rel_heights = c(2,1),
#   rel_widths = c(1,2)
# )
#   
  # ggdraw() + draw_plot(p.output)
...
```{r show-model-categorization-3D-plots-best-performing-bar-comparison, fig.ncol = 3, fig.show='hold',fig.align='center', out.width= c("33%"), out.height=c("20%"), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Model predictions in accuracy')}

filename <- make_plot_compare_models.AA(compare_model_label = "best-performing")
knitr::include_graphics(filename)
```

```{r show-model-categorization-3D-plots-best-performing, fig.ncol = 3, fig.show='hold',fig.align='center',out.width= c(rep("33%",9)), out.height=c(rep("20%",4)), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Representations: L1-accented', 'Decision making: L1-accented', 'Normalization: L1-accented', 'Representations: L2-accented', 'Decision making: L2-accented', 'Normalization: L2-accented', 'Representations: difference', 'Decision making: difference', 'Normalization: difference')}
