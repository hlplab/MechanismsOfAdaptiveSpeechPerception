...
best_performing_parameters.normalization <- readRDS(get_path(paste0("../models/best_performing_parameters.normalization_", example_label,".rds"))) # TO-DO: double check these files are saved
...
bias.parameters.select = d.AA.bias %>% filter(posterior.lapse_rate %in% posterior.lapse_rate[which.min(abs(posterior.lapse_rate - best_performing_parameters.bias$par[1]))] & 
...
compare_models.AA(
  data1 <- get_representation_models_for_plot(
    prior_kappa = prior_kappa.selected,
    prior_nu = prior_nu.selected),
 data2 <- get_decision_making_models_for_plot(
    lapse_rate = lapse_rate.selected,
    beta_pi = beta_pi.selected),
 data3 <- get_normalization_models_for_plot(
