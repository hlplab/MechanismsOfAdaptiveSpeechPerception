...
compare_models.AA <- function(data1, data2, data3, data.exposure, data.test, VOT_range, f0_range, n_points) {
  ...
  p.output <-  plot_grid(prow, ncol = 1, rel_heights = c(.1, 1))
  ...
  # ggsave(paste0('../figures/plotly/p.model_comparison_', compare_model_label, '1.png'), plot = p.accuracy1 + theme(legend.position="none"), width = 2*(base.width + .5), height = 2*(base.height + .5), dpi = 300)
  # ggsave(paste0('../figures/plotly/p.model_comparison_', compare_model_label, '2.png'), plot = p.accuracy2 + theme(legend.position="none"), width = 2*(base.width + .5), height = 2*(base.height + .5), dpi = 300)
  # ggsave(paste0('../figures/plotly/p.model_comparison_', compare_model_label, '3.png'), plot = p.accuracy3 + theme(legend.position="right"), width = 2*(base.width*1.2 + .5), height = 2*(base.height + .5), dpi = 300)
  # 
  # 
  # prow <-
  #   plot_grid(
...
```{r show-model-categorization-3D-plots-similar-accuracy, fig.ncol = 1, fig.show='hold',fig.align='center', out.width= c(rep("99%",4)), out.height=c("15%",rep("20%", 3)),  fig.cap="(ref:show-model-categorization-3D-plots-similar-accuracy)", fig.subcap=c('Model predictions in accuracy','Predicted categorization functions after L1-accented exposure', 'Predicted categorization functions after L2-accented exposure', 'Differences in categorization functions between exposure conditions'), eval=FALSE}
filename = c()
for (p in 1:length(model_names)) {
  filename[p] = file.path(get_path('../figures/plotly/'), paste0('p.3d.categorization.model_', model_names[p], '_L1-accented', '_', compare_model_label, '.png'))
  filename[p + length(model_names)] = file.path(get_path('../figures/plotly/'), paste0('p.3d.categorization.model_', model_names[p], '_L2-accented', '_', compare_model_label, '.png'))
  ...
  p1.L1 <- ggdraw() + draw_image(filename[1]) + theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  p2.L1 <- ggdraw() + draw_image(filename[2]) + theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  p3.L1 <- ggdraw() + draw_image(filename[3]) + theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  p.L1 <- plot_grid(p1.L1, p2.L1, p3.L1, ncol = 3)
  ggsave(paste0('../figures/plotly/p.3d.categorization_', conditions.AA[1], '.png'), plot = p.L1, width = 2*(base.width*3 + .5), height = 2*(base.height + .5), dpi = 300)
  
  p1.L2 <- ggdraw() + draw_image(filename[4])
  p2.L2 <- ggdraw() + draw_image(filename[5])
  p3.L2 <- ggdraw() + draw_image(filename[6])
  p.L2 <- plot_grid(p1.L2, p2.L2, p3.L2, ncol = 3)
  ggsave(paste0('../figures/plotly/p.3d.categorization_', conditions.AA[2], '.png'), plot = p.L2, width = 2*(base.width*3 + .5), height = 2*(base.height + .5), dpi = 300)
  # 
  p1.diff <- ggdraw() + draw_image(filename[7])
  p2.diff <- ggdraw() + draw_image(filename[8])
  p3.diff <- ggdraw() + draw_image(filename[9])
  p.diff <- plot_grid(p1.diff, p2.diff, p3.diff, ncol = 3)
  ggsave('../figures/plotly/p.3d.categorization_difference.png', plot = p.diff, width = 2*(base.width*3 + .5), height = 2*(base.height + .5), dpi = 300)

  filenew = c()
  filenew[1] <- file.path(get_path(paste0('../figures/plotly/p.model_comparison_', compare_model_label, '.png')))
  filenew[2] <-  file.path(get_path(paste0('../figures/plotly/p.3d.categorization_', conditions.AA[1], '.png')))
  filenew[3] <-  file.path(get_path(paste0('../figures/plotly/p.3d.categorization_', conditions.AA[2], '.png')))
  filenew[4] <-  file.path(get_path( '../figures/plotly/p.3d.categorization_difference.png'))

knitr::include_graphics(filenew)
```
...
```{r show-model-categorization-3D-plots-best-performing-bar-comparison, fig.ncol = 3, fig.show='hold',fig.align='center', out.width= c("99%"), out.height=c("20%"), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Model predictions in accuracy'), eval=FALSE}
filename <-  file.path(get_path( '../figures/plotly/p.model_comparison_best-performing.png'))
knitr::include_graphics(filename)
```

```{r show-model-categorization-3D-plots-best-performing-old, fig.ncol = 3, fig.show='hold',fig.align='center',out.width= c(rep("33%",9)), out.height=c(rep("20%",4)), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Representations: L1-accented', 'Decision making: L1-accented', 'Normalization: L1-accented', 'Representations: L2-accented', 'Decision making: L2-accented', 'Normalization: L2-accented', 'Representations: difference', 'Decision making: difference', 'Normalization: difference'),eval=FALSE}
filename <- make_plot_compare_models.AA(compare_model_label = "best-performing")
knitr::include_graphics(filename)
```

```{r show-model-categorization-3D-plots-best-performing-DEBUG, fig.ncol = 3, fig.show='hold',fig.align='center', out.width= c("100%", rep("33%",9)), out.height=c(rep("20%",5)), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Predictions from three change models: representations, decision making and normalization', 'Representations: L1-accented', 'Decision making: L1-accented', 'Normalization: L1-accented', 'Representations: L2-accented', 'Decision making: L2-accented', 'Normalization: L2-accented', 'Representations: difference', 'Decision making: difference', 'Normalization: difference'), eval=FALSE}
filename <- make_plot_compare_models.AA(compare_model_label = "best-performing")

filename <- c(file.path(get_path( '../figures/plotly/p.model_comparison_best-performing.png')), filename)
knitr::include_graphics(filename)
```

```{r show-model-categorization-3D-plots-best-performing-DEBUG2, fig.ncol = 3, fig.show='hold',fig.align='center',out.width= c(rep("33%",9), "100%"), out.height=c(rep("20%",3), "30%"), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Representations: L1-accented', 'Decision making: L1-accented', 'Normalization: L1-accented', 'Representations: L2-accented', 'Decision making: L2-accented', 'Normalization: L2-accented', 'Representations: difference', 'Decision making: difference', 'Normalization: difference', 'Predicted accuracy'), eval=FALSE}
filename <- make_plot_compare_models.AA(compare_model_label = "best-performing")

filename <- c(filename, file.path(get_path( '../figures/plotly/p.model_comparison_best-performing.png')))
knitr::include_graphics(filename)
```

```{r show-model-categorization-3D-plots-best-performing-DEBUG3, fig.ncol = 3, fig.show='hold',fig.align='center',out.width= c(rep("33%",12)), out.height=c(rep("15%",3), rep("15%", 9)), fig.cap="(ref:show-model-categorization-3D-plots-best-performing)", fig.subcap=c('Predicted accuracy: DEBUG3', 'Predicted accuracy: Decision making', 'Predicted accuracy: Normalization', 'Representations: L1-accented', 'Decision making: L1-accented', 'Normalization: L1-accented', 'Representations: L2-accented', 'Decision making: L2-accented', 'Normalization: L2-accented', 'Representations: difference', 'Decision making: difference', 'Normalization: difference'),eval=FALSE}
filename = c()
for (p in 1:length(model_names)) {
  filename[p] = file.path(get_path(paste0('../figures/plotly/p.model_comparison_', compare_model_label, p, '.png')))
}

filename <- c(filename, make_plot_compare_models.AA(compare_model_label = "best-performing"))
knitr::include_graphics(filename)
```
