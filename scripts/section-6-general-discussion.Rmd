...
compare_models.AA <- function(data1, data2, data3, data.exposure, data.test, VOT_range, f0_range, n_points) {
  ...
  for (j in 1:length(model_names)){
    ...
    for (i in 1:length(conditions.AA)){
      ...
      p.3d.categorization <- plot_3D.categorization(df.resp, width = p_width, height = p_height) %>%
        layout(
          margin = margin,
          scene = list(
            camera = list(
    ...
    p.3d.categorization.diff <- plot_3D.categorization.diff(df.diff, width = p_width, height = p_height) %>%
      layout(
        margin = margin,
        scene = list(
          camera = list(
