# General discussion
Listeners' ability to adaptively change their interpretation of the speech signal as a function of recent exposure is now understood to play a central role in spoken language comprehension. This insight is in large parts the result of research over the last two decades. In the present study we have focused on two of those lines of research that have been influential in shaping researchers' understanding of adaptive changes in speech perception: perceptual recalibration and accent adaptation. Our computational review of these two approaches suggests that the signature results of these two paradigms do not distinguish between three explanations that have quantitatively different implications for theories of speech perception, linguistic representations, and the relation between social and linguistic cognition. We find that low-level, pre-linguistic signal transformation/normalization or changes in response biases during decision-making are sufficient to explain signature results that are often tied to changes of representations. In what follows, we discuss insights emerging from the current finding, and then we close with recommendations for future experiments on speech perception. <!--[CK: I think we said all of this in the intro. The GD is super long as it is. I suggest that we leave this out for now] The arguments we present are strongly influenced by a tradition of studies that have made explicit--and thereby testable--the assumptinos applied to the interpretation of experiments on speech perception. This includes a long tradition of research on normalization [for review, see @apfelbaum-mcmurray2015; @weatherholtz-jaeger2016; @johnson-sjerps2021] and, more recently, comparisons between these approaches [@adank2014; @cole2010; @hoffmanbion-escudero2007; @mcmurray-jongman2011]. It also includes models of representational changes [@clayards2008; @kleinschmidt-jaeger2015; @kleinschmidt-jaeger2016cogsci; @winter-lancia2013] and comparisons of their variants [@hitzcenko-feldman2016; @theodore-monto2019; @tan2021], as well as comparisons between representational and normalization accounts [@apfelbaum-mcmurray2015; @kleinschmidt2018; @kleinschmidt2019; @xie2021cognition]. The framework in Section \@ref(sec:framework) combines insights from these lines of work with linking hypotheses for the specific task (categorization) employed in many experiments on speech perception [@wichmann-hill2001; see also models of lexical decision making for spoken inputs, @bosch2015; @nenadic2020]. We believe that the field would stand to gain substantially from increased familiarity with these works.--> 

## What follows from our results (and what does not)?
Our case studies suggest that one cannot draw strong conclusions about a presence or absence of representational changes from experiments on perceptual recalibration or accent adaptation. This indeterminacy has potentially far-reaching implications: the assumption that recent exposure can shape linguistic representations, or at least their weighting or selection [@xie2018jasa], is central to theories of speech perception [for review, see @hay2019; @kleinschmidt-jaeger2015; @sjerps2019]. Evidence supporting this possibility has influenced linguistic theories as well as theories about the interface between social and linguistic cognition [for review, see @foulkes-hay2015; @sumner2014]. This does not, of course, mean that normalization, representation, and decision-making mechanisms always predict similar response patterns---they do not. Nor does it mean that the three mechanisms cannot be distinguished from each other experimentally. Before we turn to ways in which this can be achieved, we first discuss other results that speak to the mechanisms underlying changes in speech perception.

One important finding is that the inferred physiology [@krauss2002] or social identity of a talker can affect how her speech is processed and interpreted [e.g., regional origin, @hay-drager2010; @niedzielski1999; sex, @johnson1999; @strand1999; age, @walker-hay2011; @skoogwaller2015; and individual identity, @nygaard1994; @remez2018]. These and other effects of the situational context [e.g., being in a car, @hay2017] suggest that listeners have implicit representations that encode expectations about talkers and types of talkers. These findings do, however, leave open whether these expectations relate to pre-linguistic normalization or to linguistic categories [see also discussion of "relativization" in @apfelbaum-mcmurray2015, p. 936-938]. They also leave open to what extent recent experience affects speech perception through the same mechanisms that underlie the effects of inferred physiology or social identity. This includes the question of whether and how recent experiences can lead to the learning of new linguistic representations.

Other findings might more directly speak to these questions. We briefly discuss findings we consider particularly informative. To anticipate, however, none of these findings decisively argues for one or the other mechanisms. For example, findings that non-speech stimuli can affect the perception of subsequently played speech stimuli [@holt2005; @holt2006; @huang-holt2011] are compatible with pre-linguistic normalization accounts but seem to be harder to explain through changes in representations. These findings have, however, been challenged [@pitt2016], and accounts of them in terms of representational changes or decision-making are possible. Similarly, there are results that seem to be difficult to explain through C-CuRE normalization or changes in responses biases. Perhaps one of the most important studies in this regard is @clayards2008 and subsequent replications and extensions [@bejjanki2011; @nixon2016; @theodore-monto2019]. Clayards and colleagues found that categorization responses can be affected by changes of the category variances, without changes in the category means. Specifically, participants that were exposed to /b/ and /p/ categories with larger variance along VOT exhibited more shallow categorization slopes than participants who were exposed to /b/ and /p/ categories with smaller variance (but the same means as the other condition). 
...
### Dense and targeted sampling of the stimulus space
...
f0_range = c(0, limits$ymax)
n_points = 100
cue_names = c("VOT", "f0")
model_names = c("Representations", "Decision-making", "Normalization")
category.contrasts = c("/d/", "/t/")
# Set image size
p_width = 900
p_height = 900

d.df.diff.all <- data.frame()
for (j in 1:length(model_names)){
  ...
  d.df.diff <- as.data.frame(df.diff$d_diff)

  colnames(d.df.diff) <- df.diff$x
  d.df.diff %<>%
    mutate(rows = row_number()) %>%
    gather(VOT, diff, -rows)

  d.df.diff.rows <- as.data.frame(df.diff$y)
  colnames(d.df.diff.rows) = "f0"
  d.df.diff.rows %<>%
    mutate(rows = row_number())

  d.df.diff %<>%
    left_join(., d.df.diff.rows, by = "rows") %>%
    mutate(VOT = as.numeric(VOT))
  
  
p.2D.heatmap <- ggplot() + 
     geom_tile(data = d.df.diff,
         aes(x = VOT, y = f0, fill = diff), alpha = 1) +
     scale_fill_gradientn(name = 'Diff. in posterior\nlog-odds of /d/', colours = c("red", "lightgrey","blue"), values = c(0,0.5,1)) +
   # geom_point(data = d.AA.test.L2.rep %>%
   #              rename(Category = Item.Intended_category), aes(x = VOT, y = f0, color = Category), alpha = 0.3, inherit.aes = FALSE) +
 ...
 # facet_wrap(~Type) +
  ...
  save_figure_or_not(p.2D.heatmap, paste0('p.2d.categorization.difference.model_', model_names[j], '.png'))

  d.df.diff.all <- bind_rows(d.df.diff.all, d.df.diff %>% mutate(model = model_names[j]))
    
}

d.AA.test.L2 <- d.AA.test %>%
  mutate(Speech = ifelse(grepl("L1", Condition), "L1-accented", "L2-accented")) %>%
  filter(Speech == "L2-accented") %>%
  rename(f0= f0_Mel) %>%
  droplevels()

d.AA.test.L2.rep <- rbind(d.AA.test.L2 %>%
                            mutate(Type = "Normalization"),
                          d.AA.test.L2 %>%
                            mutate(Type = "Representations"),
                          d.AA.test.L2 %>%
                            mutate(Type = "Decision-making")) %>%
  mutate(Type = factor(Type, levels = c("Normalization", "Representations", "Decision-making")))

ggplot() + 
     geom_tile(data = d.df.diff.all %>%
                 mutate(Type = model) %>%
                 mutate(Type = factor(Type, levels = c("Normalization", "Representations", "Decision-making"))),
         aes(x = VOT, y = f0, fill = diff), alpha = 1) +
     scale_fill_gradientn(name = 'Diff. in posterior\nlog-odds of /d/', colours = c("red", "lightgrey","blue"), limits = c(-25,25), values = c(0,0.5,1)) +
    geom_point(data = d.AA.test.L2.rep %>%
                 rename(Category = Item.Intended_category), aes(x = VOT, y = f0, color = Category), alpha = 0.3, inherit.aes = FALSE) +
  scale_color_manual(values = colors.category) +
  facet_wrap(~Type) +
...
```{r compare-AA-models-results-between-three-mechanisms-L2-accented-exposure, eval = FALSE, warning=FALSE}
df.L2 = vector(mode = "list", length = 3)

for (j in 1:length(model_names)){
  data = d.model.compare %>%
    filter(model == model_names[j])
  
  for (i in 2:length(conditions.AA)){
    output.AA <- prepare_3D.categorization_from_results(data, VOT_range, f0_range, n_points)
  ...
  # show differences between the three models in their predictions for L2-accented condition in log odds space

d.df.diff.all <- data.frame()
for (c1 in 1:2){
  for (c2 in 2:3){
  df.diff = list()
  df.diff$d_diff = qlogis(df.L2[[c1]]$d_prop) - qlogis(df.L2[[c2]]$d_prop)
  df.diff$x = df.resp$x
  df.diff$y = df.resp$y
  ...
  #     margin = margin,
  #     scene = list(
  #       zaxis = list(title = "Diff. in posterior<br>log-odds of /d/", titlefont = list(size = 22), range = c(-15,15)),
  #       camera = list(
  #         eye = list(x = -0.5, y = -2.5, z = 0.1))))
  # 
  # save_figure_or_not(p.3d.categorization.diff, paste0('p.3d.categorization.L2.difference.between_', model_names[c1], '_', model_names[c2], '.png'))
  # 
  # 
  # 
  ...
  #               cmax = 15
  # ) %>%
  #   colorbar(title = 'Diff. in posterior\nlog-odds of /d/') %>%
  #   layout(
  ...
  d.df.diff.all <- bind_rows(d.df.diff.all, d.df.diff %>% mutate(Type = paste0(model_names[c1], "_", model_names[c2])), d.df.diff)
                               
 # ggplot(d.df.diff, aes(VOT, f0, fill= diff)) +
 #     geom_tile(alpha = 0.8) +
 #     scale_fill_gradientn(colours = c("red", "lightgrey","blue"), values = c(0,0.5,1))
  # +
  #  # ggnewscale::new_scale_fill() +
  #   geom_point(data = d.AA.test %>%
  #          mutate(Speech = ifelse(grepl("L1", Condition), "L1-accented", "L2-accented")) %>%
  #          filter(Speech == "L2-accented") %>%
  #          rename(f0= f0_Mel) %>%
  #          mutate(Speech = paste0("Test: ", Speech)), aes(x = VOT, y = f0, label = as.numeric(factor(ItemID))), alpha = 0.5)
  #  stat_ellipse(data = d.AA.exposure %>% filter(Condition == "L2-accented"), aes(x= VOT,y = f0_Mel, color = Item.Category), level = .95, alpha = 0.3)+
  #   scale_color_manual("Category", values = colors.category) 

  # d.AA.exposure %>% rename(f0= f0_Mel) %>%
  #   filter(Condition == "L2-accented") %>%
  #   ggplot(aes(x= VOT,y = f0, fill = Item.Category)) +
  #    # stat_ellipse(level = .95, alpha = 0.3) + 
  #    # scale_fill_manual("Category", values = colors.category) +
  #    # ggnewscale::new_scale_fill()  +
  #   geom_tile(data = d.df.diff, aes(x=VOT, y=f0, fill= diff), alpha = 0.5) +
  #   scale_fill_gradientn(colours = c("red", "lightgrey","blue"), values = c(0,0.5,1)) 
...
d.AA.test.L2.rep <- rbind(d.AA.test.L2 %>%
                            mutate(Type = "Representations_Normalization"),
                          d.AA.test.L2 %>%
                            mutate(Type = "Representations_Decision-making"),
                          d.AA.test.L2 %>%
                            mutate(Type = "Decision-making_Normalization")) %>%
  mutate(Type = factor(Type, levels = c("Representations_Normalization", "Representations_Decision-making", "Decision-making_Normalization")))
  

ggplot() + 
     geom_tile(data = d.df.diff.all %>%
      #   filter(Type %in% c("Representations_Normalization", "Representations_Decision-making", "Decision-making_Normalization")) %>%
          filter(Type %in% c("Decision-making_Normalization")) %>%
         mutate(Type = factor(Type, levels = c("Representations_Normalization", "Representations_Decision-making", "Decision-making_Normalization"))), 
         aes(x = VOT, y = f0, fill = diff), alpha = 1) +
     scale_fill_gradientn(name = 'Diff. in posterior\nlog-odds of /d/', colours = c("red", "lightgrey","blue"), values = c(0,0.5,1)) +
   geom_point(data = d.AA.test.L2.rep %>%
...
### Data analysis beyond overall accuracy and speed 
...
Indeed, there are direct connections between the model we have employed here and lapsing logistic regression. For example, two Gaussian categories along a phonetic continuum result in linear effect of the continuum on the posterior log-odds of the categories if the two categories have identical variance, but linear + quadratic if the two categories have unequal variance [e.g., @kleinschmidt-jaeger2015; @kronrod2016]. This prediction can only be appreciated if logistic regression is employed for the analysis of 2AFC categorization tasks, rather than ANOVA or similar methods. @schertz-clare2020 provides an excellent overview of how standard regression analyses can be---and have been---used to assess the effects multiple phonetic cues on listeners’ responses.  

### Advance standards of data annotation, reporting, and sharing
...
>
> `r tufte::quote_footer('--- Frank Herbert')`
