...
tibble(
    ...
    crossing(
      Item.Category = factor(m.io.VOT.asym_var$category)) %>%
    mutate(
      x = map2(
        Item.Type,
        Item.Category, ~ case_when(
   .x == "Condition-1" & .y == experimenter.ideal_observer$category[[1]] ~ rmvnorm(1, m.io.VOT.asym_var$mu[[1]], m.io.VOT.asym_var$Sigma[[1]] / experimenter.variability_reduction),
   .x == "Condition-1" & .y == experimenter.ideal_observer$category[[2]] ~ rmvnorm(1, m.io.VOT.asym_var$mu[[2]], m.io.VOT.asym_var$Sigma[[2]] / experimenter.variability_reduction),
   .x == "Condition-2" & .y == experimenter.ideal_observer$category[[1]] ~ rmvnorm(1, m.io.VOT.asym_var$mu[[3]], m.io.VOT.asym_var$Sigma[[3]] / experimenter.variability_reduction),
   .x == "Condition-2" & .y == experimenter.ideal_observer$category[[2]] ~ rmvnorm(1, m.io.VOT.asym_var$mu[[4]], m.io.VOT.asym_var$Sigma[[4]] / experimenter.variability_reduction),
   T ~ NA_real_))) %>%
    mutate(
      Condition = Item.Type,
...
d.exposure.asym_var <- make_asymmetric_variance_exposure_design(m.io.VOT.asym_var)
...
make_asymmetric_variance_test_design = function(
  ...
  x <- 
    rbind(
      rmvnorm(
        n_test,
        m.io.VOT.asym_var %>% filter(Condition == experimenter.ideal_observer$Condition[[2]], category == experimenter.ideal_observer$category[[1]]) %>% pull(mu) %>% .[[1]],
        m.io.VOT.asym_var %>% filter(Condition == experimenter.ideal_observer$Condition[[2]], category == experimenter.ideal_observer$category[[1]]) %>% pull(Sigma) %>% .[[1]]),
      rmvnorm(
        n_test,
        m.io.VOT.asym_var %>% filter(Condition == experimenter.ideal_observer$Condition[[2]], category == experimenter.ideal_observer$category[[2]]) %>% pull(mu) %>% .[[1]],
        m.io.VOT.asym_var %>% filter(Condition == experimenter.ideal_observer$Condition[[2]], category == experimenter.ideal_observer$category[[2]]) %>% pull(Sigma) %>% .[[1]])
      )
  
  d.test <-
    tibble(
      Item.Intended_category = sort(rep(c(experimenter.ideal_observer$category[[1]], experimenter.ideal_observer$category[[2]]), n_test)),
      VOT = unlist(map(x, ~ max(min_VOT, .x[1]))),
      x = map(VOT, ~ c("VOT" = .x))) %>%
    mutate(
      Phase = "test",
...
d.test.asym_var <- make_asymmetric_variance_test_design(m.io.VOT.asym_var)

make_exposure_test_plot(d.exposure.asym_var, d.test.asym_var)

# get predictions from representational model
