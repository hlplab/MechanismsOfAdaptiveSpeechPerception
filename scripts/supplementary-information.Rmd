...
## Applying C-CuRE
We follow @mcmurray-jongman2011 and use linear regression to remove the effects of talker from each observation in the database. The original data from @chodroff-wilson2018 contained nearly three times more /d/ than /t/; this asymmetry was retained after the data cleaning procedures described above. In order to generate to a balanced dataset, we selected subsamples in two steps. First, we restricted the selection to 9 pairs of /d/- and /t/-items that had identical vowels following the stop consonant (e.g, _*d*ied_ and _*t*ime_). Although we did not include corrections for phonological context, this procedure minimized variation associated with changes in the immediate context. Second, we randomly subsampled equal number of tokens for each category within each talker from talkers who had both /d/ and /t/ observations; this number was determined by the number of tokens of the less frequent category for each talker. For instance, if a talker produced 40 /d/s and 20 /t/s, then 20 /d/ and 20 /t/ observations were randomly pulled from the data. Together, these subsampling procedures generated a total of 5632 observations (2816 per category). To avoid over-fitting, we use linear mixed effects regression rather than ordinary linear regression. Separate regressions were use to predict VOT and f0 (Mel). Both regressions used the formula:
