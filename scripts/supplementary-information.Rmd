...
# 
# temp <- m.io.VOT.asym_var.dist %>%
#   group_by(!!! syms(setdiff(names(.), c("prior","lapse_rate", "lapse_bias", "Sigma_noise")))) %>%
#       crossing(category = categories.demo, !! sym(cue.renamed) := seq(VOT_range[1], VOT_range[2], by = 0.01)) %>% 
#       ungroup() %>%
#   mutate(y = ifelse(
#         category == categories.demo[1],
