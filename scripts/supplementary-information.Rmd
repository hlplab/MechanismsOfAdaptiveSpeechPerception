...
```{r proposed-experiment-asymmetric-variance, fig.width=base.width*3 + 0.5, fig.height=base.height + 0.5, fig.cap="(ref:proposed-experiment-asymmetric-variance)"}
...
p.distribution.asym_var <- m.io.VOT.asym_var.dist %>%
  ...
  scale_x_continuous("VOT (standardized)") +
...
prow <- plot_grid(
    ...
    p.asym_var.representations + theme(legend.position="none"), 
    p.asym_var.normalization + theme(legend.position="none"), 
    labels = c('A)', 'B)', 'C)'), 
    align = "hv", axis = "btlr",
    ncol = 3, rel_widths = c(1/3, 1/3, 1/3))
