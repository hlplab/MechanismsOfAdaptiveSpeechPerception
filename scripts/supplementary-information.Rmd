...
```{r optimzation-results, fig.width=base.width*3 + .5, fig.height=base.height*1 + .5, fig.cap = "(ref:optimization-results)"}
...
p.bias <- 
  ...
  scale_x_continuous(
    name = ~beta[pi], 
    breaks = 10^(log10(range.beta_pi[1]):log10(range.beta_pi[2])),
...
p.normalization <- 
  ...
  aes(x = kappa.normalization, y = lapse_rate) +
  scale_x_continuous(
    name = ~kappa[0], 
    breaks = 10^(0:log10(range.prior_kappa.normalization[2])),
