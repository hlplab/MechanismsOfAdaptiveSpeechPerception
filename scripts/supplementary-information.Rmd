...
# Visualization
##--------------
# VOT_range <- c(0, 110) # if using raw or c-cured cues

VOT_range = c(-3,3) # if using standardized cues
# cue distribution
p.distribution.asym_var <- m.io.VOT.asym_var.dist %>%
...
m.ia.VOT.asym_var.representations <-
  crossing(
...
# get predictions from best-performing representational model
m.ia.VOT.asym_var.normalization <-
  crossing(
    prior_kappa.normalization = best_performing_parameters.normalization.asym_var$par[1], 
    lambda = lambdas.demo) %>%
  mutate(
    posterior = map2(
      prior_kappa, 
...
```{r plot-changes-in-representations-DEBUG, message=FALSE, warning=FALSE, eval = FALSE}
