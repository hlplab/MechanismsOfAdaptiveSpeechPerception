...
If you want to knit the document into a PDF file, you will some additional packages. First, make sure that your version of the R package \texttt{tinytex} is up-to-date. Then run \texttt{tinytex::install_tinytex()} to install latex on your computer and/or link it to R. As of 12/20/2022, it was also necessary to run \texttt{tinytex::tlmgr_install('biber')}, though that step might become unnecessary in the future. Next, download the IPA font [SIL Doulos](https://software.sil.org/doulos/download/), which is required for the IPA symbols in the paper. 
...
# Optimization steps for Case Study 2
...
# Exposure condition 1
 mu_A.exposure1 <- m.VOT_f0_MVG %>% filter(category == "/d/") %>% pull(mu) %>% .[[1]] %>% .[1]
 Sigma_A.exposure1 <- m.VOT_f0_MVG %>% filter(category == "/d/") %>% pull(Sigma) %>% .[[1]] %>% .[1]
 mu_B.exposure1 <- m.VOT_f0_MVG %>% filter(category == "/t/") %>% pull(mu) %>% .[[1]] %>% .[1]
 Sigma_B.exposure1 <- m.VOT_f0_MVG %>% filter(category == "/t/") %>% pull(Sigma) %>% .[[1]] %>% .[1]
...
m.io.VOT.con2 <- m.io.VOT.con1 # create two conditions that have identical category means, but switched category variance
...
d.chodroff_wilson.selected %<>%
  ...
  group_by(Talker) %>%
  mutate(VOT_scaled = scale_variable(VOT))
...
## Trying to use the plot_expected_categories_density1D function here, didn't work
# colnames(m.io.VOT.asym_var) <- c("category","m", "S","prior", "lapse_rate", "lapse_bias", "Sigma_noise", "Condition")
# m.io.VOT.asym_var.belief <- m.io.VOT.asym_var %>%
#   mutate(kappa = 4, nu = 4)
# plot_expected_categories_density1D(m.io.VOT.asym_var.belief, d.asym_var.exposure %>% mutate(category = Item.Category), d.asym_var.test %>% mutate(category = Item.Intended_category), categories = categories.demo)
...
```{r model-changes-in-representations-asym-var, message=FALSE, warning=FALSE, eval = FALSE}
...
# change if needed to change the variance for demo presentation
# m.io.VOT.con1$Sigma[1] = 20
# m.io.VOT.con1$Sigma[2] = 400
...
#   summarise(mean = mean(VOT), var= var(VOT))

d.asym_var.test <- make_asymmetric_variance_test_design(m.io.VOT.asym_var)
#sanity check
# d.asym_var.test %>%
#   group_by(Condition, Item.Intended_category) %>%
#   summarise(mean = mean(VOT), var= var(VOT))
...
m.ia.VOT.asym_var <-
  crossing(
    prior_kappa = best_performing_parameters.representations.asym_var$par[1], 
    prior_nu = best_performing_parameters.representations.asym_var$par[2],
    lambda = lambdas.demo) %>%
  mutate(
    posterior = map2(
      prior_kappa, 
...
d.representations.asym_var <- 
  ...
  crossing(
    m.ia.VOT.asym_var %>% 
      filter(lambda == 0) %>%
      filter(Condition == "Condition-1") %>% # DEBUG: simplify this. m.ia.VOT.asym_var does not need Condition-2?
      select(-Condition) %>%
...
# sanity check
# d.representations.asym_var.results <- d.representations.asym_var %>%
#   group_by(Condition, !!! syms(my_groups)) %>%
#   mutate(response.overall = mean(response)) %>%
#   group_by(Condition, !!! syms(my_groups), category, response.overall) %>%
#   summarise(response = mean(response))

```

```{r demonstrate-changes-in-representations-asym-var, fig.width=base.width*2*1.5, fig.height=base.height + 1, fig.cap="(ref:demonstrate-changes-in-representations-SI)", eval=FALSE}
...
m.io.VOT.asym_var <- left_join(m.io.VOT.asym_var.mu, m.io.VOT.asym_var.Sigma) %>%
  mutate(likelihood1 = map2(`mu_/d/`, `Sigma_/d/`, ~ function(x) dnorm(x, .x, .y^.5)),
         likelihood2 = map2(`mu_/t/`, `Sigma_/t/`, ~ function(x) dnorm(x, .x, .y^.5))
         )


# cue distribution
p.distribution.asym_var <- m.io.VOT.asym_var %>%
  filter(Condition == "Condition-2") %>%
  ggplot(aes(x = !! sym(cue.renamed))) +
  geom_line(
    data = . %>% 
      distinct(Condition, likelihood1, likelihood2) %>%
      crossing(category = categories.demo, !! sym(cue.renamed) := seq(VOT_range[1], VOT_range[2])) %>%
      ungroup() %>%
      mutate(y = ifelse(
    ...
    aes(y = y, color = category, alpha = Condition))+
...
# plot posterior category distribution under best-performing parameterization
# d.representations.asym_var %>%
#   mutate(
...
#     sigma2 = map(posterior, ~ get_expected_Sigma_from_S(.x$S, .x$nu)[2]) %>% unlist()) %>% # obtain the likelihood for each test token
#   mutate(
...
#   filter(observationID ==1 & Condition == "Condition-2") %>%
#   ggplot(aes(x = !! sym(cue.renamed))) +
#   # cue distribution
#   geom_line(
#     data = . %>% 
#       distinct(Condition, !!! syms(my_groups), posterior.likelihood1, posterior.likelihood2, observationID) %>%
#       crossing(category = categories.demo, !! sym(cue.renamed) := seq(VOT_range[1], VOT_range[2])) %>%
#       ungroup() %>%
#       mutate(y = ifelse(
#         category == categories.demo[1],
#         map2(posterior.likelihood1, !! sym(cue.renamed), .f = mycall) %>% unlist(),
#         map2(posterior.likelihood2, !! sym(cue.renamed), .f = mycall) %>% unlist())),
#     aes(y = y, color = category, 
#         alpha = factor(paste(!! sym(my_groups[2]), sep = ", ")), 
#         group = paste(category, !!! syms(my_groups)))) +
#   scale_x_continuous("VOT (msec)") +
#   scale_y_continuous("Density") +
#   scale_color_manual("Category", values = colors.voicing) +
...
```{r plot-test-results-asym-var-DEBUG, eval= FALSE}
...
```
```{r model-changes-in-normalization-asym-var-SCALED_CUES-DEBUG, message=FALSE, warning=FALSE, eval = FALSE}
```




```{r model-changes-in-representations-asym-var-SCALED_CUES-DEBUG, message=FALSE, warning=FALSE, eval = FALSE}
# 
# set.seed(5612)
# 
# categories.demo <- c("/d/", "/t/")
...
#   cues = cue.renamed) %>%
#   filter(category %in% categories.demo) %>%
#   lift_MVG_to_MVG_ideal_observer(prior = bias.demo, lapse_rate = 0, lapse_bias = bias.demo, Sigma_noise = matrix(80, nrow = 1, dimnames = list(cue.renamed)))
# 
# # change if needed to change the variance for demo presentation
# m.io.VOT.con1$Sigma[1] = 20
# m.io.VOT.con1$Sigma[2] = 400
...
# m.io.VOT.con2$Sigma[1] = m.io.VOT.con1$Sigma[2] 
# m.io.VOT.con2$Sigma[2] = m.io.VOT.con1$Sigma[1] 
# m.io.VOT.asym_var.scaled <- bind_rows(m.io.VOT.con1 %>% mutate(Condition = "Condition-1"), 
#                                m.io.VOT.con2 %>% mutate(Condition = "Condition-2"))
# 
# 
# 
...
#   ggplot(aes(x = !! sym(cue.renamed))) +
#   geom_line(
#     data = m.io.VOT.asym_var %>% 
#       distinct(Condition, likelihood1, likelihood2) %>%
#       crossing(category = categories.demo, !! sym(cue.renamed) := seq(min(d.chodroff_wilson.selected$VOT_scaled), max(d.chodroff_wilson.selected$VOT_scaled), by = 0.01)) %>%
#       ungroup() %>%
#       mutate(y = ifelse(
#         category == categories.demo[1],
#         map2(likelihood1, !! sym(cue.renamed), .f = mycall) %>% unlist(),
